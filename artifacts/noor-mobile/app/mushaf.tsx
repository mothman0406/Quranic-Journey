...
function AyahActionSheet({
  ...
  onCopyAyah: (target: PageAyahTarget) => void;
  onOpenCredits: () => void;
    ...
    return (
      <>
        ...
        <View style={styles.ayahMenuGroup}>
          ...
          <Pressable style={styles.ayahMenuRow} onPress={() => onCopyAyah(target)}>
            <Ionicons name="copy-outline" size={18} color="#374151" />
            <Text style={styles.ayahMenuRowText}>Copy ayah</Text>
          </Pressable>
          <Pressable style={styles.ayahMenuRow} onPress={onOpenCredits}>
            <Ionicons name="information-circle-outline" size={18} color="#2563eb" />
            <Text style={styles.ayahMenuRowText}>Mushaf credits</Text>
          </Pressable>
        </View>
...
export default function MushafScreen() {
  ...
  useEffect(() => {
    ...
    async function loadInitialPage() {
      try {
        const raw = await AsyncStorage.getItem(GENERAL_LAST_PAGE_KEY);
        if (cancelled) return;
        const savedPage =
          raw != null && Number.isInteger(Number(raw))
            ? clampMushafPage(Number(raw))
            : 1;
    ...
    async function loadBookmarks() {
      try {
        const raw = await AsyncStorage.getItem(GENERAL_BOOKMARKS_KEY);
    ...
    async function loadRecentReads() {
      try {
        const raw = await AsyncStorage.getItem(GENERAL_RECENT_KEY);
        if (cancelled) return;
    ...
    async function loadAudioSettings() {
      ...
      try {
        const raw = await AsyncStorage.getItem(GENERAL_AUDIO_SETTINGS_KEY);
        if (cancelled) return;
    ...
    async function loadViewMode() {
      try {
        const savedMode = await AsyncStorage.getItem(GENERAL_VIEW_MODE_KEY);
        if (cancelled) return;
        setMushafViewMode(
          requestedViewMode ?? (savedMode === "scroll" ? "scroll" : "swipe"),
        );
      } catch {
        if (!cancelled) setMushafViewMode(requestedViewMode ?? "swipe");
      }
    }
  ...
  }, [requestedInitialPage, requestedViewMode]);
  ...
  useEffect(() => {
    ...
    AsyncStorage.setItem(GENERAL_AUDIO_SETTINGS_KEY, JSON.stringify(audioSettings)).catch(() => {
      // Best-effort only.
    });
  ...
  useEffect(() => {
    ...
    setRecentReads((current) => {
      ...
      AsyncStorage.setItem(GENERAL_RECENT_KEY, JSON.stringify(next)).catch(() => {
        // Best-effort only.
      });

      return next;
    });
  }, [currentPage, currentSurah.name, currentSurah.number, initialPage]);
  ...
  function updateMushafViewMode(mode: MushafViewMode) {
    ...
    AsyncStorage.setItem(GENERAL_VIEW_MODE_KEY, mode).catch(() => {
      // Best-effort only.
    });
  }
  ...
  async function savePage(page: number) {
    ...
    try {
      await AsyncStorage.setItem(GENERAL_LAST_PAGE_KEY, String(target));
  ...
  function toggleBookmark(page = currentPage) {
    ...
    setBookmarkedPages((current) => {
      ...
      AsyncStorage.setItem(GENERAL_BOOKMARKS_KEY, JSON.stringify(next)).catch(() => {
  ...
  function copyAyah(target: PageAyahTarget) {
    setSelectedAyah(null);
  ...
  function openCreditsFromAyahSheet() {
    setSelectedAyah(null);
    setAttributionOpen(true);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <AyahActionSheet
        ...
        onCopyAyah={copyAyah}
        onOpenCredits={openCreditsFromAyahSheet}
