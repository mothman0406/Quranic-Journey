...
_Last updated: April 29, 2026 (Phase 2I.2d Ready to Recite hardware QA passed; Phase 2I.2e Just Get Tested next)_
...
- Phase 2I.2d hardware QA notes: Mohammad reported QA passed. The next slice is Phase 2I.2e Just Get Tested shortcut; do not start 2I.2f, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2e is implemented, documented, validated, and hardware-tested or Mohammad explicitly reorders.
- Exact next checklist:
  1. Start Phase 2I.2e: add a `Just Get Tested` shortcut for review-only sessions on the mobile Memorize setup/discovery surface.
  2. The shortcut should skip memorization playback entirely and route straight to the existing Recitation Check surface for the full review-only assignment.
  3. Preserve review-only all-or-nothing save semantics, daily-progress behavior, 2I.2a completed-to save semantics for normal sessions, 2I.2b leave semantics, and 2I.2d Ready to Recite behavior for normal session-complete.
  4. Preserve overview cards, dashboard Memorize entry behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, Recitation Check, and mark-complete persistence.
  5. Validate only with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; do not run the full root typecheck.
  6. Do not touch `artifacts/noor-path`, do not tighten recite matcher behavior, do not touch tajweed implementation except to document it as backlogged, and do not add native dependencies.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up, 2I.2a Pause & Save, 2I.2b Save & Leave, 2I.2c Recitation Check, and 2I.2d Ready to Recite are hardware-tested; Just Get Tested, setup phase, View in Full Mushaf, bookmark/resume, Skip repeat / Skip ayah, ayah end-marker sheet, word translation tooltip + WBW audio, recite-mode page auto-flip, Show Word / Skip Word affordances, multi-page recite navigation, confetti, persisted memorization defaults, and the visible memorization settings/defaults page still need implementation.
...
## 🔜 NEXT — Phase 2I.2e Just Get Tested Shortcut

1. Add a `Just Get Tested` shortcut for review-only Memorize assignments on the active mobile app.
2. It should skip memorization playback and route directly to the existing Recitation Check surface for the full review-only assignment.
3. Preserve review-only all-or-nothing save semantics, 2I.2a normal completed-to saves, 2I.2b leave semantics, 2I.2d Ready to Recite normal-session behavior, and daily-progress behavior.
4. Validate only with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; do not run the full root typecheck.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `2545acd4d5cad19fd5aa9ce0aa4b2bd75c790c1d` before this Phase 2I.2d hardware-QA docs update. After this docs commit, all four refs should be synced to the final Phase 2I.2d hardware-confirmed docs HEAD. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, Phase 2I.2a Pause & Save hardware-tested, Phase 2I.2b Save & Leave hardware-tested, Phase 2I.2c Recitation Check hardware-tested, and Phase 2I.2d Ready to Recite hardware-tested. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2e Just Get Tested, then remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
