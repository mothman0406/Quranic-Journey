...
_Last updated: April 29, 2026 (Phase 2I.2a Pause & Save implemented locally; hardware QA pending)_
...
- Exact next checklist:
  ...
  8. After 2I.2 ships and is hardware-tested, do Phase 2I.3 (memorization defaults parity) and 2I.4 (discovery polish: tajweed notes accordion, ayah-strength review tone on surah rows).
  9. Then Phase 2J Review never-empty/ahead-day parity, then Phase 2K Full Quran/Mushaf Bayaan parity. Do not start TestFlight readiness until 2I, 2J, and 2K are all complete and hardware-tested.
  10. Do not tighten recite matcher behavior. Do not touch tajweed implementation unless only documenting it as missing/backlogged.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up are hardware-tested, and 2I.2a Pause & Save is locally implemented pending hardware QA; leave flow, recitation check phase, "Ready to Recite?" choice, setup phase, View in Full Mushaf, bookmark/resume, Skip repeat / Skip ayah, ayah end-marker sheet, word translation tooltip + WBW audio, recite-mode page auto-flip, Show Word / Skip Word affordances, multi-page recite navigation, confetti, persisted memorization defaults, and the visible memorization settings/defaults page still need implementation.
...
| 2I.2a Pause & Save modal with completed-to ayah picker | Implemented locally Apr 29: the session action opens Pause & Save; normal sessions ask "How far did you get?" with a bounded completed-to ayah stepper and Current ayah / Full range shortcuts; review-only sessions use an all-or-nothing Finish Recitation path with `Go to Recitation Check`; the existing rating/save semantics now rate only ayahs through the selected completed-to value and daily progress uses the selected completed-to ayah. | `memorization.tsx` | Existing memorization/daily-progress routes | Medium session-state risk | Local mobile typecheck and `git diff --check` passed; hardware QA pending with partial-save/full-range/review-only checklist |
...
## 🔜 NEXT — Phase 2I.2a Hardware QA

1. Start a normal memorization session, tap `Pause & Save`, pick a smaller completed-to ayah, continue to rating, choose Needs Work / Good / Excellent, save, and confirm only ayahs through the selected completed-to value are rated/memorized.
2. Confirm partial Pause & Save keeps today's assignment `in_progress`, preserves the same `Today's work`, advances `Current work` only as far as the selected completed-to ayah allows, and refreshes the dashboard after returning.
3. Repeat with `Full range` selected and confirm full-range save behavior still matches the Phase 2I.1 save semantics.
4. Test a review-only `Recitation Focus`/`Recitation` assignment: confirm the session button says `Finish Recitation`, the Pause & Save sheet does not show the partial picker, `Go to Recitation Check` opens rating, and saving applies to the full assigned range.
5. Confirm `Keep Practicing` closes both Pause & Save/rating sheets without saving.
6. Smoke search/filter/resume/session flow, including Needs Work -> Practice loading.
7. After Phase 2I.2a hardware QA passes, document the result, then continue Phase 2I.2b Save & Leave / Leave without saving / Keep practicing modal.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both should be synced to the current Phase 2I.2a local-validation/docs commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, and Phase 2I.2a Pause & Save locally implemented/validated. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2a hardware QA, then Phase 2I.2b/remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
