...
_Last updated: April 29, 2026 (Phase 2I.1 hardware QA pending; Bug A hardware-tested/shipped; Bug B diagnostic logs in place pending Mohammad's repro capture)_
...
Step 0: collect Mohammad's `[noor-mem]` diagnostic logs for the `Next up` loading death spiral, then fix Bug B and remove the temporary logs in the same patch (see `Hardware bugs` section below) so QA does not get derailed. Then:
...
- **Memorize `Next up` card → loading death spiral.** Diagnostic logs are in place in `artifacts/noor-mobile/app/child/[childId]/memorization.tsx` pending Mohammad's repro capture. Every temporary log is prefixed `[noor-mem]` and covers `beginSession`, the Step 1 dashboard fetch effect (including `[noor-mem] step1 UNEXPECTED dashboard-fetch path`), the Step 2 verses+timings effect, the `[playingVerseNumber]` cleanup/autoplay effect, and `playVerse`/`Audio.Sound.createAsync`. Local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Next action: Mohammad taps `Next up`, captures the `[noor-mem]` console lines, and pastes them back. Do not patch blindly; remove all diagnostic logs in the same commit as the eventual root-cause fix. JS-only; no tajweed or matcher changes.
