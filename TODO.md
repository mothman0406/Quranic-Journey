...
_Last updated: April 29, 2026 (Phase 2I.1 overview cards plus save semantics/rating follow-up are implemented and locally validated; hardware QA is pending, and beta is still blocked by 2I/2J/2K parity before TestFlight readiness)_
...
- Phase 2I.1 current-work status follow-up: the `Current work`/`Recitation Focus` overview card now also reads today's daily progress. When the frozen current-work range has been completed, it shows `Complete`, uses the completed green card tone, and changes the action to `Practice`, matching the same frozen-card semantics as `Today's work`.
- Intra-surah review clarification: this is already implemented in the backend memorization workflow and surfaced in the mobile Memorize section. `buildSurahMemorizationWorkflow` splits a long surah into page-target chunks; for surahs taking 2-3 memorization days it schedules the new chunks plus a final whole-surah test; for exactly 4 chunks it schedules four new chunks plus a `Full Cumulative Recitation` labeled `Whole Surah Test`; for surahs taking more than 4 chunks it adds recurring intra-surah recitation days after each block (`Cumulative Recitation` for the latest block and `Full Cumulative Recitation` from the start through that block) before the final whole-surah test. The dashboard returns these items as `todaysPlan.newMemorization` with `isReviewOnly: true`, and mobile 2I.1 shows them as `Recitation Focus`/`Recitation` in the overview cards. What remains for 2I.2 is the richer web-style session UX around those review-only days: explicit Pause & Save / Save & Leave / Leave without saving, a clearer Finish Recitation affordance, and View in Full Mushaf.
- Exact next checklist:
  1. Mohammad hardware-tests Phase 2I.1: open Memorize from dashboard; confirm `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; complete today's/current work; choose Needs Work/Good/Excellent and save; confirm both frozen cards stay on the same assignment/range and show done/in-progress as appropriate; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today remains today; return to dashboard and confirm dashboard refresh; smoke search/filter/resume/session flow.
...
- Phase 2I.1 memorization overview cards plus save semantics/rating follow-up are locally implemented: the Memorize route now has `Today's work`, `Current work`/`Recitation Focus`, and `Next up` cards backed by dashboard `todaysPlan.newMemorization`, `todayProgress`, and `upNextMemorization`, including backend-generated intra-surah review/test work for multi-day surahs. Session completion now requires a Needs Work/Good/Excellent rating before saving unioned ayah progress and today's daily-progress state. Hardware QA is pending.
...
2. Memorization: mobile has the core learner, 2H.6 discovery, locally implemented 2I.1 overview cards, and the 2I.1 save semantics/rating follow-up. Intra-surah review for multi-day surahs is already scheduled by the backend and surfaced in Memorize as `Recitation Focus`/`Recitation` cards: 2-3 chunk surahs get a final whole-surah test, 4-chunk surahs get a full cumulative whole-surah test, and >4 chunk surahs get recurring block/full cumulative recitation days before the final whole-surah test. Web still defines the required next work after hardware QA: explicit setup/defaults, Pause & Save/leave flows, clearer finish-recitation actions, teacher/app recitation check, View in Full Mushaf, and resume/bookmark clarity. Do not touch recite matcher behavior. Do not pursue tajweed beyond documenting it as backlogged.
...
5. Settings/targets/parent controls: mobile now has a parent Settings surface for targets, practice minutes, visibility flags, profile entry, profile creation/deletion, and JS-only memorization defaults; web still has deeper custom-goal and content-management surfaces.
...
- Branch-sync note: `main` + `feature/main-working-branch` were synced at `cbe9937` after Phase 2I.1 cards. After this save-semantics docs sync, both branches should be synced to the current docs HEAD. `safe-cumulative` was temporary archaeology and can be ignored.
...
2. Complete today's work, choose Needs Work/Good/Excellent, and save.
3. Confirm today's card and current-work card stay on the same assignment/range and show done/in-progress instead of advancing to tomorrow's work.
4. Confirm saved ayah strength renders red/yellow/green where applicable.
5. Complete extra work if possible and confirm `Next up` can change while today's card remains today's card.
6. Return to dashboard and confirm dashboard refresh.
7. Smoke search/filter/resume/session flow.
8. Keep tajweed as backlog-only unless Mohammad explicitly reopens it, and do not tighten recite matcher behavior.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both should be synced to the current Phase 2I.1 save-semantics docs commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, and Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up locally implemented. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.1 hardware QA, then Phase 2I.2/2I.3 and Phase 2J/2K blockers before TestFlight readiness. |
</content>
