...
_Last updated: April 30, 2026 (Phase 2I.2i Skip repeat / Skip ayah local validation passed; hardware QA pending)_
...
1. Commit/sync this local-validation update to both branches.
2. Hardware-test Skip Repeat in single-phase repeat sessions and cumulative review, plus Skip Ayah in single-phase, at the final ayah, and during cumulative review.
3. Preserve existing behavior for review-only assignments, `Just Get Tested`, bookmark/resume, Pause & Save, Save & Leave, Ready to Recite, Recitation Check, View in Full Mushaf, daily-card freeze semantics, ayah/page modes, cumulative review, audio highlighting, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
4. Do not start 2I.2j or later work until 2I.2i hardware QA passes.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; at Phase 2I.2i implementation start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `2b8524f3f0cbbe8c5019e27d47682b6338f5edd3`. After validation/docs commit, all four refs should be synced to the final Phase 2I.2i local-validation HEAD. `safe-cumulative` was temporary archaeology and can be ignored. |
