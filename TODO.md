...
_Last updated: April 29, 2026 (Phase 2I.2b Save & Leave implemented locally; hardware QA next)_
...
- Exact next checklist:
  1. Commit and push the Phase 2I.2b local-validation implementation/docs, then sync `feature/main-working-branch` to `main`.
  2. Mohammad should hardware-test Phase 2I.2b on iPhone: active-session Back shows the leave sheet; `Keep Practicing` dismisses without losing place/audio where possible; normal-session `Save & Leave` opens completed-to picker then rating and saves only selected ayahs; review-only `Save & Leave` remains all-or-nothing; `Leave without saving` returns to Memorize overview with no progress/daily-progress save; iOS swipe/back gesture behavior should be smoke-tested.
  3. After 2I.2b hardware QA passes, update both docs and continue Phase 2I.2c Recitation Check phase with score card.
  4. Do not start Phase 2I.2d+, Phase 2I.3/2I.4, Phase 2J, Phase 2K, or TestFlight readiness until 2I.2b is hardware-tested or Mohammad explicitly reorders.
  5. Do not tighten recite matcher behavior. Keep tajweed implementation backlog-only unless Mohammad explicitly reopens it.
...
| 2I.2b Save & Leave / Leave without saving / Keep practicing modal | Implemented locally Apr 29: active-session Back opens a leave confirmation sheet; `usePreventRemove` intercepts route-removal/back gestures where supported; `Save & Leave` routes into the existing Pause & Save completed-to/rating flow; review-only remains all-or-nothing; `Leave without saving` stops recite/audio, unloads sound, clears active session state, and returns to Memorize overview without progress POSTs; `Keep Practicing` dismisses only. | `memorization.tsx` | None | Medium navigation risk | Mobile typecheck and `git diff --check` passed; Mohammad hardware QA pending |
...
## 🔜 NEXT — Phase 2I.2b Hardware QA, Then 2I.2c

1. Commit and sync the Phase 2I.2b local-validation work to `main` plus `feature/main-working-branch`.
2. Mohammad should hardware-test the 2I.2b leave modal on iPhone: active-session Back, `Keep Practicing`, normal `Save & Leave`, review-only `Save & Leave`, `Leave without saving`, and a smoke test of iOS swipe/back gesture interception.
3. If hardware QA passes, update both docs and continue Phase 2I.2c Recitation Check phase with score card.
4. Keep tajweed as backlog-only unless Mohammad explicitly reopens it, and do not tighten recite matcher behavior.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both were synced to `1fe3b7836bcc5c8c6b3f5b87e09435e965f2cc5b` at Phase 2I.2b start and should be synced to the final Phase 2I.2b local-validation commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, Phase 2I.2a Pause & Save hardware-tested, and Phase 2I.2b Save & Leave implemented locally with mobile typecheck passing. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2b hardware QA, then Phase 2I.2c Recitation Check, remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
