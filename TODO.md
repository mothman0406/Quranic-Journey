...
_Last updated: April 29, 2026 (Phase 2I.2d Ready to Recite local validation passed; hardware QA pending)_
...
- Exact next checklist:
  1. Commit on `main`, push `origin main`, fast-forward `feature/main-working-branch` onto `main`, push `origin feature/main-working-branch`, and switch back to `main`.
  2. Mohammad should hardware-test: finish a normal session, see `Ready to Recite?`, tap `Recite to Teacher` and save through Recitation Check; finish another session, tap `Recite to NoorPath`, complete recite mode, see the score card, and save; smoke Save & Leave / Leave without saving / Keep Practicing.
  3. After Mohammad reports hardware QA, update this file and `PHASE_2D_HANDOFF.md` to mark Phase 2I.2d hardware QA passed.
  4. Do not start 2I.2e/2I.2f, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until Mohammad confirms 2I.2d hardware QA.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up, 2I.2a Pause & Save, 2I.2b Save & Leave, and 2I.2c Recitation Check are hardware-tested; "Ready to Recite?" is implemented and locally validated with hardware QA pending; setup phase, View in Full Mushaf, bookmark/resume, Skip repeat / Skip ayah, ayah end-marker sheet, word translation tooltip + WBW audio, recite-mode page auto-flip, Show Word / Skip Word affordances, multi-page recite navigation, confetti, persisted memorization defaults, and the visible memorization settings/defaults page still need implementation.
...
| 2I.2d "Ready to Recite?" choice modal at session end | Implemented locally Apr 29: at session-complete (last verse done, optional cumulative review done), mobile shows a `Ready to Recite?` sheet. `Recite to Teacher` jumps straight to the existing Recitation Check surface with source `teacher`; `Recite to NoorPath` starts existing recite mode from the assigned first ayah and routes back to Recitation Check with the score card when recitation finishes. | `memorization.tsx` | None | JS-only, medium UX risk | Mobile typecheck and `git diff --check` passed; hardware QA should hit session-complete and confirm both routes land in check correctly |
...
## 🔜 NEXT — Phase 2I.2d Hardware QA

1. Commit/sync the locally validated Phase 2I.2d implementation on both branches.
2. Hardware QA: finish a normal session and confirm the `Ready to Recite?` sheet appears after the last ayah/repeat/cumulative review; `Recite to Teacher` opens Recitation Check with source `teacher`; `Recite to NoorPath` starts recite mode from the assigned first ayah and returns to Recitation Check with the score card.
3. Smoke existing 2I.2a/2I.2b/2I.2c behavior: Pause & Save completed-to picker, Save & Leave, Leave without saving, Keep Practicing, review-only all-or-nothing save, and daily-progress behavior.
4. Keep tajweed as backlog-only unless Mohammad explicitly reopens it, do not tighten recite matcher behavior, and do not add native dependencies.
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, Phase 2I.2a Pause & Save hardware-tested, Phase 2I.2b Save & Leave hardware-tested, and Phase 2I.2c Recitation Check hardware-tested. Phase 2I.2d Ready to Recite is implemented and locally validated with hardware QA pending. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; after 2I.2d QA continue remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
