...
_Last updated: April 29, 2026 (Phase 2I.1 overview cards are implemented, but memorization save semantics/rating must be fixed before hardware QA; beta is still blocked by 2I/2J/2K parity before TestFlight readiness)_
...
| Slice | Goal | Likely files | Backend/API deps | Risk | QA |
|---|---|---|---|---|---|
| 2I.1 Memorization overview cards | Implemented locally Apr 29, but needs immediate save-semantics follow-up before hardware QA: added top three mobile cards for `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; next fix must ensure save/rating marks today's work done via daily progress instead of visually advancing today's card. | `app/child/[childId]/memorization.tsx`, `src/lib/memorization.ts` if needed | Existing `GET /dashboard`, `GET /memorization`, `GET /surahs`, `POST /daily-progress`, `POST /memorization`; no generated edits expected | JS-only | Local mobile typecheck + `git diff --check` passed for cards; save/rating fix and hardware QA pending |
...
## 🔜 NEXT — Phase 2I.1 Save Semantics/Rating Follow-Up

1. Read frozen web `artifacts/noor-path/src/pages/quran-memorize.tsx` around `dailyProgressMutation`, `saveMutation`, `matchesTodayMemTarget`, and the `phase === "check"` rating UI.
2. Fix mobile `artifacts/noor-mobile/app/child/[childId]/memorization.tsx` so completing a session asks for Needs Work/Good/Excellent, saves that quality rating, preserves existing memorized ayahs, and posts daily progress for today's target. Today's card should stay today's work and show done/in-progress; `Next up` may change when extra work is completed.
3. Keep Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf as the next broader 2I.2 work after this follow-up unless the smallest rating/save screen naturally overlaps.
4. Validate with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`, then Mohammad hardware-tests the three cards plus rating/save behavior.
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, and Phase 2I.1 memorization overview cards locally implemented. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is the Phase 2I.1 save-semantics/rating follow-up, then hardware QA, then Phase 2I.2/2I.3 and Phase 2J/2K blockers before TestFlight readiness. |
</content>
