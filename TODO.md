...
_Last updated: April 29, 2026 (Phase 2I.2e Just Get Tested locally validated; hardware QA pending)_
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up, 2I.2a Pause & Save, 2I.2b Save & Leave, 2I.2c Recitation Check, and 2I.2d Ready to Recite are hardware-tested; 2I.2e Just Get Tested is locally validated and awaiting hardware QA; setup phase, View in Full Mushaf, bookmark/resume, Skip repeat / Skip ayah, ayah end-marker sheet, word translation tooltip + WBW audio, recite-mode page auto-flip, Show Word / Skip Word affordances, multi-page recite navigation, confetti, persisted memorization defaults, and the visible memorization settings/defaults page still need implementation.
...
| 2I.2e "Just Get Tested" shortcut for review-only sessions | Locally validated Apr 29: review-only overview assignments expose a `Just Get Tested` shortcut that skips memorization playback and routes directly to the existing Recitation Check surface for the full review-only assignment range. | `memorization.tsx` | None | JS-only | Mobile typecheck and `git diff --check` passed; hardware QA pending |
...
## 🔜 NEXT — Hardware QA Phase 2I.2e Just Get Tested Shortcut

1. Commit the locally validated 2I.2e implementation/docs on `main`, push `origin main`, fast-forward `feature/main-working-branch` onto `main`, push `origin feature/main-working-branch`, and switch back to `main`.
2. Hardware-test a review-only assignment day: confirm `Just Get Tested` appears, tap it, land directly in Recitation Check for the full assignment range, rate/save, and confirm daily progress/dashboard update.
3. Smoke 2I.2a/2I.2b/2I.2c/2I.2d behavior after the shortcut.
