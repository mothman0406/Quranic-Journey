...
_Last updated: April 29, 2026 (Phase 2I.2f pre-session setup hardware QA passed; next is Phase 2I.2g bookmark/resume per session)_
...
- Exact next checklist:
  ...
  3. Add a "Resume where you left off" card to Memorize discovery that hydrates the bookmark, restores the setup/session settings, and starts at the saved ayah without breaking today's/current/next freeze semantics.
...
## 🔜 NEXT — Phase 2I.2g Bookmark / Resume Per Session

1. Persist a per-child memorization bookmark to AsyncStorage as the active session changes: surah, fromAyah, toAyah, currentAyah, repeatCount, autoAdvance, cumulativeReview, reviewRepeatCount, review-only flag if needed, and savedAt.
2. Add a "Resume where you left off" card to Memorize discovery that hydrates the bookmark and starts the session at the saved ayah with saved settings.
3. Clear or replace stale bookmarks when a session is completed, intentionally left without saving, or superseded by a new session.
4. Preserve all hardware-tested 2I.2a-2I.2f behavior and today's/current/next freeze semantics.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; at Phase 2I.2f hardware-QA docs start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `dc7bf1436b86924762be271467d74f854cc08db1`. After this docs commit, all four refs should be synced to the final Phase 2I.2f hardware-confirmed docs HEAD. `safe-cumulative` was temporary archaeology and can be ignored. |
