...
- Exact next checklist:
  1. Mohammad should hardware-test: finish a normal session, see `Ready to Recite?`, tap `Recite to Teacher` and save through Recitation Check; finish another session, tap `Recite to NoorPath`, complete recite mode, see the score card, and save; smoke Save & Leave / Leave without saving / Keep Practicing.
  2. After Mohammad reports hardware QA, update this file and `PHASE_2D_HANDOFF.md` to mark Phase 2I.2d hardware QA passed.
  3. Do not start 2I.2e/2I.2f, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until Mohammad confirms 2I.2d hardware QA.
...
1. Hardware QA: finish a normal session and confirm the `Ready to Recite?` sheet appears after the last ayah/repeat/cumulative review; `Recite to Teacher` opens Recitation Check with source `teacher`; `Recite to NoorPath` starts recite mode from the assigned first ayah and returns to Recitation Check with the score card.
2. Smoke existing 2I.2a/2I.2b/2I.2c behavior: Pause & Save completed-to picker, Save & Leave, Leave without saving, Keep Practicing, review-only all-or-nothing save, and daily-progress behavior.
3. Keep tajweed as backlog-only unless Mohammad explicitly reopens it, do not tighten recite matcher behavior, and do not add native dependencies.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `94f56029ca5fc5697b07ffbe5d7f2d90152331bb` at Phase 2I.2d implementation start, then both branches were pushed to implementation commit `0fc6a1d`. After this docs-sync commit, all four refs should be synced to the final local-validation HEAD. `safe-cumulative` was temporary archaeology and can be ignored. |
