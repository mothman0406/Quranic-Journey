...
_Last updated: April 29, 2026 (Phase 2I.1 memorization overview cards are locally implemented; hardware QA pending; beta is still blocked by 2I/2J/2K parity before TestFlight readiness)_
...
- Exact next checklist:
  ...
  2. If QA passes, record the hardware result in this file and `PHASE_2D_HANDOFF.md`.
...
2. Memorization: mobile has the core learner, 2H.6 discovery, and locally implemented 2I.1 overview cards awaiting hardware QA. Web still defines the required next work: explicit setup/defaults, Pause & Save/leave flows, teacher/app recitation check, View in Full Mushaf, and resume/bookmark clarity. Do not touch recite matcher behavior. Do not pursue tajweed beyond documenting it as backlogged.
...
| Slice | Goal | Likely files | Backend/API deps | Risk | QA |
|---|---|---|---|---|---|
| 2I.1 Memorization overview cards | Implemented locally Apr 29: added the web-style top three mobile Memorize cards for `Today's work`, `Current work`/`Recitation Focus`, and `Next up`, with full/current/next assignment ranges, status, and existing session-engine routing. | `app/child/[childId]/memorization.tsx` | Existing `GET /dashboard`, `GET /memorization`, `GET /surahs`; no generated edits | JS-only | Local mobile typecheck + `git diff --check` passed; Mohammad hardware QA pending |
...
- Branch-sync note: `main` + `feature/main-working-branch` were synced at `561910d` before Phase 2I.1. After this Phase 2I.1 implementation/docs sync, both branches should be synced to the current local-validation HEAD. `safe-cumulative` was temporary archaeology and can be ignored.
...
## 🔜 NEXT — Hardware QA for Phase 2I.1, then Phase 2I.2

1. Mohammad hardware-tests Phase 2I.1 on iPhone: open Memorize from dashboard; confirm `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; verify each card has understandable assignment/status/range; start today's work; start/resume current work; start next-up if available; return to dashboard and confirm dashboard refresh; smoke existing search/filter/resume/session flow.
2. If QA passes, record the hardware result in `TODO.md` and `PHASE_2D_HANDOFF.md`.
3. Start Phase 2I.2 Pause & Save/session action parity: Pause & Save, Save & Leave, Leave without saving, review-only Finish Recitation handling, and View in Full Mushaf.
4. Preserve ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both should be synced to the current Phase 2I.1 local-validation commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
