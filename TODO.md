...
_Last updated: April 29, 2026 (Phase 2I.2a hardware QA passed; Phase 2I.2b Save & Leave next)_
...
- Exact next checklist:
  1. Start Phase 2I.2b: add a leave confirmation modal for active memorization sessions with `Save & Leave`, `Leave without saving`, and `Keep practicing`.
  2. Hook the visible header Back action and any feasible active-session back/abort path. If Expo Router gesture interception is too risky, document the limitation and cover the visible Back path first.
  3. `Save & Leave` should route into the existing 2I.2a Pause & Save / rating flow. Normal sessions should still choose the completed-to ayah; review-only sessions stay all-or-nothing.
  4. `Leave without saving` should stop/unload audio, clear the active session, return to the Memorize overview, and avoid any memorization/daily-progress POST.
  5. `Keep practicing` should dismiss the modal and leave audio/session state intact where possible.
  6. Preserve overview cards, dashboard Memorize entry behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, Pause & Save semantics, and mark-complete persistence.
  7. Validate only with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`.
  8. Update both docs after implementation with branch/SHA, validation result, QA checklist, and the next 2I.2 sub-row.
  9. After 2I.2 ships and is hardware-tested, do Phase 2I.3 (memorization defaults parity) and 2I.4 (discovery polish: tajweed notes accordion, ayah-strength review tone on surah rows).
  10. Then Phase 2J Review never-empty/ahead-day parity, then Phase 2K Full Quran/Mushaf Bayaan parity. Do not start TestFlight readiness until 2I, 2J, and 2K are all complete and hardware-tested.
  11. Do not tighten recite matcher behavior. Do not touch tajweed implementation unless only documenting it as missing/backlogged.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up and 2I.2a Pause & Save are hardware-tested; leave flow, recitation check phase, "Ready to Recite?" choice, setup phase, View in Full Mushaf, bookmark/resume, Skip repeat / Skip ayah, ayah end-marker sheet, word translation tooltip + WBW audio, recite-mode page auto-flip, Show Word / Skip Word affordances, multi-page recite navigation, confetti, persisted memorization defaults, and the visible memorization settings/defaults page still need implementation.
...
## 🔜 NEXT — Phase 2I.2b Save & Leave Modal

1. Start Phase 2I.2b: add a leave confirmation modal for active memorization sessions with `Save & Leave`, `Leave without saving`, and `Keep practicing`.
2. Hook the visible header Back action and any feasible active-session back/abort path. If Expo Router gesture interception is too risky, document the limitation and cover visible Back first.
3. Route `Save & Leave` into the existing 2I.2a Pause & Save / rating flow. Normal sessions should still choose the completed-to ayah; review-only sessions stay all-or-nothing.
4. Make `Leave without saving` stop/unload audio, clear the active session, return to the Memorize overview, and avoid memorization/daily-progress POSTs.
5. Make `Keep practicing` dismiss the modal and leave audio/session state intact where possible.
6. Preserve overview cards, dashboard Memorize entry behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, Pause & Save semantics, and mark-complete persistence.
7. Validate only with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; do not run the full root typecheck.
8. Update both `TODO.md` and `PHASE_2D_HANDOFF.md` after implementation with branch/SHA, validation, QA checklist, and the next 2I.2 sub-row.
9. Keep tajweed as backlog-only unless Mohammad explicitly reopens it, and do not tighten recite matcher behavior.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both should be synced to the current Phase 2I.2a hardware-confirmed docs commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, and Phase 2I.2a Pause & Save hardware-tested. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2b Save & Leave / Leave without saving / Keep practicing, then remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
