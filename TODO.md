...
_Last updated: April 29, 2026 (Phase 2I.2g bookmark/resume implemented and locally validated; hardware QA still required before 2I.2h)_
...
| 2I.2g Bookmark / resume per session | Implemented and locally validated Apr 29: per-child session bookmarks now persist to AsyncStorage as active session state changes with surah, From/To, current ayah, repeat count, auto-advance, cumulative review, review repeat count, review-only flag, page range, and `savedAt`. Memorize discovery shows a distinct `Resume where you left off` card that hydrates the bookmark and starts at the saved ayah. Successful saves clear the bookmark, `Leave without saving` discards it, and newer sessions replace it. | `memorization.tsx`, `src/lib/settings.ts` | None | JS-only | Mobile typecheck and `git diff --check` passed; Mohammad hardware QA pending: quit mid-session, return, resume at saved ayah/settings, save clears, discard clears |
...
## 🔜 NEXT — Commit/Sync + Hardware-Test Phase 2I.2g Bookmark / Resume

1. Commit/sync `main` and `feature/main-working-branch`.
2. Hardware QA on iPhone: quit mid-session, return to Memorize, see `Resume where you left off`, tap it, land at the saved ayah with saved range/settings, save successfully, and confirm the card clears.
3. Hardware QA also needs `Leave without saving` to clear the bookmark intentionally, plus a newer session replacing the previous bookmark.
4. Do not start 2I.2h or later phases until 2I.2g is synced and hardware-tested. Keep tajweed backlogged, do not tighten recite matcher behavior, do not edit `artifacts/noor-path`, and do not add native dependencies.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; at Phase 2I.2g implementation start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `22c766a9e3e2ba86ca9546954304812ef02d36c3`. After the 2I.2g implementation/docs commit, all four refs should be synced to the final local-validation HEAD. `safe-cumulative` was temporary archaeology and can be ignored. |
