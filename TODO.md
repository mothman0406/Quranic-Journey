...
_Last updated: April 29, 2026 (Phase 2I.2c Recitation Check implemented locally; hardware QA next)_
...
- Exact next checklist:
  1. Hardware-test Phase 2I.2c on iPhone: Pause & Save normal session -> completed-to picker -> Recitation Check -> Needs Work / Good / Excellent -> Save Progress.
  2. Hardware-test `Save & Leave` normal session -> completed-to picker -> Recitation Check -> Save Progress; confirm `Leave without saving` still returns to Memorize without memorization/daily-progress POSTs and `Keep Practicing` still dismisses only.
  3. Hardware-test review-only Finish Recitation / Save & Leave: confirm the Recitation Check saves the full assigned range all-or-nothing.
  4. Hardware-test session-complete and recite-mode completion: normal audio completion should land on Recitation Check; recite-mode completion should land on Recitation Check with the Recite to NoorPath score card.
  5. Smoke-test overview cards, dashboard Memorize entry, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, and Needs Work -> Practice loading.
  6. Do not start Phase 2I.2d+, Phase 2I.3/2I.4, Phase 2J, Phase 2K, or TestFlight readiness until 2I.2c is hardware-tested or Mohammad explicitly reorders.
  7. Do not tighten recite matcher behavior. Keep tajweed implementation backlog-only unless Mohammad explicitly reopens it.
...
| 2I.2c Recitation Check phase with score card | Implemented Apr 29, hardware QA pending: mobile now shows a dedicated full-screen Recitation Check surface after Pause & Save / Save & Leave completed-to selection, review-only Finish Recitation, normal session-complete, and recite-mode completion. It preserves Needs Work / Good / Excellent quality values `2 / 4 / 5`, normal completed-to save semantics, review-only all-or-nothing saves, and shows a Recite to NoorPath score card when entered from recite mode. | `memorization.tsx` | Existing memorization/daily-progress routes | Medium state risk | Mobile typecheck and `git diff --check` passed; hardware-test all check entry points and save/leave semantics |
...
## 🔜 NEXT — Phase 2I.2c Hardware QA

1. Hardware-test the new Recitation Check surface from normal Pause & Save: completed-to picker -> Recitation Check -> Needs Work / Good / Excellent -> Save Progress.
2. Hardware-test `Save & Leave`, `Leave without saving`, and `Keep Practicing` to confirm 2I.2b leave semantics still hold.
3. Hardware-test review-only Finish Recitation / Save & Leave to confirm the full assigned range remains all-or-nothing.
4. Hardware-test session-complete and recite-mode completion; recite-mode completion should show the Recite to NoorPath score card.
5. Smoke-test overview cards, dashboard Memorize entry behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, mark-complete persistence, and Needs Work -> Practice loading.
6. Keep tajweed as backlog-only unless Mohammad explicitly reopens it, and do not tighten recite matcher behavior.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both were synced to `1b72b036d075a13d366b33b97994cdefd314e998` before the Phase 2I.2c Recitation Check implementation and should be synced to the final Phase 2I.2c local-validation docs/implementation commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, Phase 2I.2a Pause & Save hardware-tested, Phase 2I.2b Save & Leave hardware-tested, and Phase 2I.2c Recitation Check implemented with local validation passed. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2c hardware QA, then remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
