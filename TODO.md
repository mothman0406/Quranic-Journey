...
_Last updated: April 30, 2026 (Phase 2I.2i Skip repeat / Skip ayah committed/synced after local validation; hardware QA pending)_
...
| 2I.2i Skip repeat / Skip ayah toolbar controls | Implemented locally Apr 30: active sessions show `Skip Repeat` and `Skip Ayah` under Prev/Play/Next. Single-phase `Skip Repeat` advances repeat passes or finishes the current ayah's repeat cycle; cumulative `Skip Repeat` advances to the next cumulative pass or exits after the final pass. Single-phase `Skip Ayah` advances to the next ayah or existing completion flow; cumulative `Skip Ayah` exits/advances review without corrupting the single-phase cursor. | `memorization.tsx` | None | JS-only, medium state risk | Mobile typecheck and `git diff --check` passed; Mohammad hardware QA pending |
...
## 🔜 NEXT — Hardware-test Phase 2I.2i Skip Repeat / Skip Ayah

1. Hardware-test Skip Repeat in single-phase repeat sessions and cumulative review, plus Skip Ayah in single-phase, at the final ayah, and during cumulative review.
2. Preserve existing behavior for review-only assignments, `Just Get Tested`, bookmark/resume, Pause & Save, Save & Leave, Ready to Recite, Recitation Check, View in Full Mushaf, daily-card freeze semantics, ayah/page modes, cumulative review, audio highlighting, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
3. Do not start 2I.2j or later work until 2I.2i hardware QA passes.
