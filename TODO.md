...
_Last updated: April 30, 2026 (Phase 2I.2h View in Full Mushaf hardware QA passed; next is Phase 2I.2i Skip repeat / Skip ayah)_
...
## 🔜 NEXT — Phase 2I.2i Skip Repeat / Skip Ayah

1. Add Skip Repeat and Skip Ayah controls to the active mobile memorization controls without changing the recite matcher or adding native dependencies.
2. Mirror the web intent for repeat skipping: in single-phase playback, Skip Repeat should advance the repeat pass or finish the current ayah's repeat cycle; in cumulative review, Skip Repeat should advance the cumulative pass/step.
3. Mirror the web intent for ayah skipping: in single-phase playback, Skip Ayah should advance to the next ayah or session completion flow; in cumulative review, Skip Ayah should exit/advance cumulative review without corrupting the memorization cursor.
4. Preserve existing behavior for review-only assignments, `Just Get Tested`, bookmark/resume, Pause & Save, Save & Leave, Ready to Recite, Recitation Check, View in Full Mushaf, daily-card freeze semantics, ayah/page modes, cumulative review, audio highlighting, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
5. Validate with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; then commit/sync both branches and hardware-test before starting 2I.2j or later work.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; at Phase 2I.2h hardware-QA docs start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `36622e5ad0805e491a886b4f636c900c933915ef`. After this docs commit, all four refs should be synced to the final Phase 2I.2h hardware-confirmed docs HEAD. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, Phase 2I.2a Pause & Save hardware-tested, Phase 2I.2b Save & Leave hardware-tested, Phase 2I.2c Recitation Check hardware-tested, Phase 2I.2d Ready to Recite hardware-tested, Phase 2I.2e Just Get Tested hardware-tested, Phase 2I.2f Pre-session setup hardware-tested, 2I.2g Bookmark / resume hardware-tested, 2I.2h View in Full Mushaf hardware-tested, `Next up` skip-gap behavior hardware-tested, and Today's/Current work freeze correction hardware-tested. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2i Skip repeat / Skip ayah, followed by remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
