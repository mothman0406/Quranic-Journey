...
_Last updated: April 29, 2026 (Phase 2H.2 mobile dashboard parity content complete and hardware-tested; Phase 3/TestFlight deferred until Phase 2H must-have parity is triaged/completed)_
...
- Exact next checklist:
  1. Start Phase 2H.3 settings/targets convergence: expand mobile parent settings around the existing Targets/Profile flows, including practice minutes, visibility toggles, child profile edit, daily presets, profile settings persistence, and any lightweight default session settings that can stay JS-only.
  2. Keep the existing seeded-profile flows intact: dashboard, targets, memorization, review, More, Full Quran, profile picker, profile creation, profile settings, delete child, and pull-to-refresh.
  3. Validate Phase 2H.3 with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; broaden to backend/spec checks only if backend or OpenAPI changes are truly needed.
  4. Mohammad hardware QA for Phase 2H.3: edit targets, practice minutes, visibility flags, profile fields, and any session defaults; leave/reopen settings; verify dashboard suggestions honor hide flags; verify core routes still open.
