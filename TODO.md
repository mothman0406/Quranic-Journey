...
_Last updated: April 29, 2026 (Phase 2I.2b Save & Leave hardware QA passed; Phase 2I.2c Recitation Check next)_
...
- Exact next checklist:
  1. Start Phase 2I.2c: add a separate Recitation Check phase with a score card while preserving existing 2I.2a/2I.2b save semantics.
  2. The check phase should be reachable from the existing Mark Complete/Pause & Save rating path first, then from recite-finish/session-complete paths if feasible without starting 2I.2d. Use the existing Needs Work / Good / Excellent quality values (2/4/5).
  3. Preserve normal-session completed-to selection, review-only all-or-nothing saves, daily-progress behavior, overview cards, dashboard Memorize entry behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, Leave modal behavior, and mark-complete persistence.
  4. Validate only with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; do not run full root typecheck.
  5. Do not start Phase 2I.2d+, Phase 2I.3/2I.4, Phase 2J, Phase 2K, or TestFlight readiness until 2I.2c is implemented and hardware-tested or Mohammad explicitly reorders.
  6. Do not tighten recite matcher behavior. Keep tajweed implementation backlog-only unless Mohammad explicitly reopens it.
...
## 🔜 NEXT — Phase 2I.2c Recitation Check

1. Start Phase 2I.2c: add a separate Recitation Check phase with score card after Mark Complete / Pause & Save / recite-finish entry points where feasible.
2. Reuse the existing Needs Work / Good / Excellent rating values and save semantics. Normal sessions must still honor completed-to ayah selection; review-only sessions must stay all-or-nothing.
3. Preserve the 2I.2b leave modal behavior, overview cards, dashboard Memorize entry behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
...
| Branches | `main` (deploy) + `feature/main-working-branch`; both were synced to `e1342344aa538a521321e1df101e110530df4610` before this 2I.2b hardware-QA docs update and should be synced to the final Phase 2I.2b hardware-confirmed docs commit after this handoff. `safe-cumulative` was temporary archaeology and can be ignored. |
...
| Mobile app HEAD | Phase 2D complete through Slice 5b, Phase 2E dashboard polish complete, Phase 2F targets complete, Phase 2G.1-2G.3 hardware-tested, Phase 2H.1-2H.6 hardware-tested, Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up hardware-tested, Phase 2I.2a Pause & Save hardware-tested, and Phase 2I.2b Save & Leave hardware-tested. Backend cumulative recitation scheduling was corrected during Phase 2I.1 QA. EAS dev build `cfb3f406-5fec-405a-a150-e525a96ecff2` finished and was hardware-tested by Mohammad for earlier slices. Beta is not done; next recommended work is Phase 2I.2c Recitation Check, then remaining 2I.2, 2I.3/2I.4, and Phase 2J/2K blockers before TestFlight readiness. |
</content>
