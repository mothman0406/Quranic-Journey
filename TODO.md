...
_Last updated: April 29, 2026 (Phase 2I.1 hardware QA pending; web-app parity audit refresh added missing items into Phase 2I.2/2I.3/2I.4 and surfaced two new hardware bugs that block 2I.1 QA)_
...
- Exact next checklist:
  1. Fix or diagnose the two new hardware bugs first (Memorize `Next up` loading death spiral, surah search keyboard sticking) so they don't derail the 2I.1 hardware QA pass. Both are in the new Hardware Bugs section.
  2. Mohammad hardware-tests Phase 2I.1: open Memorize from dashboard; confirm `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; confirm cumulative recitation days follow the corrected 4-new / block-cumulative / 4-new / block-cumulative / full-cumulative / final-test cadence when a multi-day surah reaches those checkpoints; complete today's/current work; choose Needs Work/Good/Excellent and save; confirm `Today's work` stays the whole assignment, `Current work` advances through unfinished items inside today's assignment, and `Current work` only shows `Complete` after the last item in today's work is done; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today remains today; return to dashboard and confirm dashboard refresh; smoke search/filter/resume/session flow.
  3. After hardware QA passes, document the result, then continue Phase 2I.2 Pause/save and session action parity, working through the 2I.2a-2I.2i sub-rows.
  4. After 2I.2 ships and is hardware-tested, do Phase 2I.3 (memorization defaults parity) and Phase 2I.4 (discovery polish: tajweed notes accordion, ayah-strength review tone on surah rows).
  5. Then Phase 2J Review never-empty/ahead-day parity, then Phase 2K Full Quran/Mushaf Bayaan parity. Do not start TestFlight readiness until 2I, 2J, and 2K are all complete and hardware-tested.
  6. Do not tighten recite matcher behavior. Do not touch tajweed implementation unless only documenting it as missing/backlogged.
...
Step 0: fix the two new hardware bugs first (see `Hardware bugs` section below) so they don't derail QA. Then:
...
- **Memorize surah search keyboard sticks open.** When searching for a surah on the Memorize page, the iOS keyboard does not dismiss when the list is scrolled, and there is no Done / dismiss affordance. The keyboard covers roughly half the screen and blocks tapping rows. Fix path: add `keyboardDismissMode="on-drag"` (iOS) and `keyboardShouldPersistTaps="handled"` to the discovery `ScrollView`; optionally add an iOS Done accessory on the `TextInput`; optionally add a small clear/dismiss button next to the search field. JS-only.
