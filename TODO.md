...
_Last updated: April 29, 2026 (Phase 2I.1 hardware QA pending; Memorize search keyboard-dismiss Bug A implemented for isolated hardware test; Next up loading Bug B still pending)_
...
- Hardware bugs found Apr 29 (block Phase 2I.1 QA): tapping the `Next up` card on the Memorize page sometimes lands on a screen that doesn't load (loading death spiral / circle of death); and on the Memorize page the surah search keyboard does not dismiss on scroll and has no Done / dismiss affordance, so it covers half the screen. Both are added to the new Hardware Bugs section below and must be fixed (or at least diagnosed) before resuming the Phase 2I.1 hardware QA pass so QA does not get derailed by them.
...
- Exact next checklist:
  1. Mohammad hardware-tests the isolated Memorize search keyboard Bug A fix: open Memorize, tap the surah search field, confirm the keyboard dismisses when dragging the list, confirm the Done/return key dismisses it, and confirm filter/row taps are reachable while the keyboard is up.
  2. After Bug A hardware test passes, diagnose/fix Bug B: Memorize `Next up` loading death spiral. Add temporary `[noor-mem]` console logs around `beginSession`, the Step 1 dashboard fetch effect, the Step 2 verses+timings effect, and the autoPlay branch of the `[playingVerseNumber]` cleanup effect; capture iPhone console output; fix the root cause; remove logs in the same commit.
  3. After both hardware bugs are cleared, Mohammad hardware-tests Phase 2I.1: open Memorize from dashboard; confirm `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; confirm cumulative recitation days follow the corrected 4-new / block-cumulative / 4-new / block-cumulative / full-cumulative / final-test cadence when a multi-day surah reaches those checkpoints; complete today's/current work; choose Needs Work/Good/Excellent and save; confirm `Today's work` stays the whole assignment, `Current work` advances through unfinished items inside today's assignment, and `Current work` only shows `Complete` after the last item in today's work is done; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today remains today; return to dashboard and confirm dashboard refresh; smoke search/filter/resume/session flow.
  4. After hardware QA passes, document the result, then continue Phase 2I.2 Pause/save and session action parity, working through the 2I.2a-2I.2i sub-rows.
  5. After 2I.2 ships and is hardware-tested, do Phase 2I.3 (memorization defaults parity) and Phase 2I.4 (discovery polish: tajweed notes accordion, ayah-strength review tone on surah rows).
  6. Then Phase 2J Review never-empty/ahead-day parity, then Phase 2K Full Quran/Mushaf Bayaan parity. Do not start TestFlight readiness until 2I, 2J, and 2K are all complete and hardware-tested.
  7. Do not tighten recite matcher behavior. Do not touch tajweed implementation unless only documenting it as missing/backlogged.
...
## 🔜 NEXT — Hardware bug fixes, then Phase 2I.1 Hardware QA

Step 0: hardware-test the isolated Memorize search keyboard fix, then diagnose/fix the `Next up` loading death spiral (see `Hardware bugs` section below) so QA does not get derailed. Then:
...
## 🐞 Hardware bugs — found Apr 29, 2026 (must clear before Phase 2I.1 hardware QA)

These were reported during the Apr 29 web→mobile parity audit conversation and block QA because they make the Memorize page hard to navigate.

- **Memorize `Next up` card → loading death spiral.** Tapping the `Next up` overview card on the Memorize page often leads to a screen that doesn't load well (loading spinner that never resolves or a blank/broken state). Repro path needed: pick a child with both today's work and an `upNextMemorization` value; from Memorize tap `Next up`; observe. Likely candidates: route param mismatch when `Next up` is review-only vs new-memorization, missing `pageStart`/`pageEnd` on the up-next response, or a session-fetch race in `memorization.tsx` when the new range overlaps the cached `displayWordsMap`. Diagnosis path: add temporary `[noor-mem]` console logs around the `beginSession` → `surahNumber` change effect and the `Step 1 dashboard fetch` effect, capture the iPhone console, and trace which step hangs. Fix should not tighten the recite matcher and should not touch tajweed. JS-only.
- **Memorize surah search keyboard sticks open.** Implemented locally Apr 29 in `artifacts/noor-mobile/app/child/[childId]/memorization.tsx`: the discovery `ScrollView` now uses `keyboardDismissMode="on-drag"` and `keyboardShouldPersistTaps="handled"`, the horizontal filter scroller also preserves handled taps, and the search `TextInput` uses an explicit Done return key that dismisses the keyboard. JS-only. Local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Hardware QA pending: Mohammad should verify the keyboard dismisses on scroll/Done and rows remain tappable.
