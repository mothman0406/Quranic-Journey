...
_Last updated: April 29, 2026 (Phase 2H.3 settings/targets convergence implemented locally; hardware QA pending; Phase 3/TestFlight deferred until Phase 2H must-have parity is triaged/completed)_
...
- Phase 2H.3 hardware QA notes: pending Mohammad iPhone QA.
- Exact next checklist:
  1. Mohammad hardware QA for Phase 2H.3: edit daily preset bundles and individual memorization/review/reading targets; confirm dashboard updates after leaving Settings and after pull-to-refresh.
  2. Edit practice minutes and content visibility flags; confirm settings persist after leaving/reopening and dashboard story/du'a suggestions appear/disappear according to `hideStories`/`hideDuas`.
  3. Open Profile Settings from Settings/More; edit name, age, and avatar; confirm profile picker/dashboard/settings reflect them; confirm delete child confirmation remains scoped to the selected child.
  4. Smoke core seeded-profile routes after settings changes: dashboard, settings/targets, memorization, review, More, Full Quran, profile picker, profile creation, profile settings, delete child, and pull-to-refresh.
  5. If Phase 2H.3 hardware QA passes, mark it complete/hardware-tested in both docs and start Phase 2H.4 review essentials parity. Keep Phase 3/TestFlight deferred until Phase 2H must-have parity is completed or explicitly moved to post-beta. Keep tajweed as documented backlog only; do not tighten recite matching; do not edit the frozen web app; do not edit generated files manually; keep future implementation JS-only unless a native rebuild is explicitly approved.
...
| 2H.3 Settings/targets convergence | Implemented locally Apr 29: converged Targets into parent Settings with practice minutes, visibility toggles, child profile entry, daily target bundles/fine tuning, JS-only memorization defaults, and immediate profile preference persistence. | `targets.tsx`, `src/lib/settings.ts`, `memorization.tsx`, dashboard/More labels | Existing `PUT /children/:id`; AsyncStorage for session defaults | JS-only; no native | Mobile typecheck + `git diff --check` passed; Mohammad hardware QA pending |
