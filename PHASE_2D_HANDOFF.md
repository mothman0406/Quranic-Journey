...
**Last updated: 2026-04-30 (Phase 2I.2i Skip repeat / Skip ayah hardware QA passed; next is Phase 2I.2j Tappable ayah end-marker; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  1. Start Phase 2I.2j Tappable ayah end-marker -> ayah sheet in the active mobile memorization session.
  2. Make the existing end-marker circle tappable in non-recite, non-blind mode and open an ayah sheet for that ayah.
  3. Include useful sheet actions without starting later slices: at minimum View in Full Mushaf at that ayah and a memorization handoff/current-ayah action if it fits existing state; keep WBW tap-tooltip/audio, recite-mode helpers, and confetti for later 2I.2 slices.
  4. Preserve review-only assignments, `Just Get Tested`, bookmark/resume, Pause & Save, Save & Leave, Ready to Recite, Recitation Check, View in Full Mushaf, Skip Repeat / Skip Ayah, daily-card freeze semantics, ayah/page modes, cumulative review, audio highlighting, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
  5. Do not start 2I.2k, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2j is implemented, documented, validated, and hardware-tested unless Mohammad explicitly reorders.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up, 2I.2a Pause & Save, 2I.2b leave semantics, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, 2I.2f Pre-session setup, 2I.2g bookmark/resume, 2I.2h View in Full Mushaf, 2I.2i Skip repeat / Skip ayah, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are hardware-tested; remaining session settings/defaults, richer ayah/word actions, recite-mode helpers, celebration states, and a visible memorization settings/defaults page still need implementation.
...
## 7. Next: Phase 2I.2j Tappable Ayah End-Marker

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, Phase 2I.2e, Phase 2I.2f, Phase 2I.2g, Phase 2I.2h, Phase 2I.2i, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are complete and hardware-tested. The next active implementation item is Phase 2I.2j Tappable ayah end-marker -> ayah sheet.
...
- **Current next** — Implement Phase 2I.2j Tappable ayah end-marker: tap the ayah end marker in non-recite/non-blind memorization mode, open a sheet for that ayah, and include focused actions such as View in Full Mushaf at that ayah.
...
2. **Check git state.** At Phase 2I.2i hardware-QA docs start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `3bcb7b01adec07efd4f201dd657c3a296bb3334a`; after this docs update they should be synced to the final docs HEAD. `safe-cumulative` can be ignored.
3. **Start Phase 2I.2j next.** Implement Tappable ayah end-marker -> ayah sheet before starting word tooltip/WBW audio or any later 2I/2J/2K/TestFlight work.
