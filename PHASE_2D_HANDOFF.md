...
**Last updated: 2026-04-29 (Phase 2I.2f pre-session setup implemented and local validation passed; hardware QA next; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  1. Mohammad hardware QA for 2I.2f: setup appears before overview/resume/surah playback; From/To bounds hold; repeat count 1-10 is honored; auto-advance advances through the selected range; cumulative review and review repeat count are honored; surah picker/navigation works for non-review setup; review-only setup keeps the assigned range locked; `Just Get Tested` still opens Recitation Check without playback.
  2. Smoke existing 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e behavior and today's/current/next freeze semantics after setup.
  3. Do not start 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2f is hardware-tested unless Mohammad explicitly reorders.
...
- Memorization session/settings parity: mobile now has Pause & Save completed-to picking, Save & Leave / Leave without saving / Keep Practicing, Recitation Check with score card, Ready to Recite, Just Get Tested, `Next up` skip-gap behavior, and Today's/Current work freeze semantics implemented and hardware-tested. Phase 2I.2f setup controls for ayah range/repeats/auto-advance/cumulative review/review repeat count are implemented and locally validated; hardware QA is next. It still needs View in Full Mushaf, bookmark/resume clarity, skip controls, richer ayah/word actions, recite-mode page helpers, celebration overlays, and a visible memorization settings page for defaults.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are implemented, validated, and hardware-tested. 2I.2f Pre-session setup is implemented and locally validated with hardware QA pending. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, Just Get Tested, daily-card freeze semantics, and mark-complete persistence. | `memorization.tsx`, `targets.tsx` copy, maybe `mushaf.tsx` route params later | Existing progress/daily-progress routes | Medium session-state risk | 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e mobile typecheck / `git diff --check` passed and Mohammad hardware QA passed; skip-gap/freeze focused API typecheck, mobile typecheck, `git diff --check`, and Mohammad hardware QA passed; 2I.2f mobile typecheck and `git diff --check` passed |
...
| **2I.2f Pre-session setup** | 🟡 local validation passed | pending | Setup appears before overview/resume/surah playback starts; includes bounded From/To, repeat count, auto-advance, cumulative review, conditional review repeat count, and surah navigation/picker; review-only setup keeps the assignment range locked and preserves Just Get Tested |
...
## 7. Next: Phase 2I.2f Hardware QA

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, Phase 2I.2e, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are complete and hardware-tested. Phase 2I.2f Pre-session setup is implemented and locally validated; hardware QA is next.
...
- **Current next** — Hand Phase 2I.2f to Mohammad for hardware QA.
- **Phase 2I next** — After 2I.2f is hardware-tested, continue memorization parity completion with View in Full Mushaf, bookmark/resume, skip controls, ayah/word actions, recite-mode helpers, confetti, discovery polish, and a visible memorization settings/defaults page unless Mohammad explicitly reorders.
...
2. **Check git state.** At Phase 2I.2f start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `107edbcbdfda92f1250bb055defa80c1dd254ede`; after the 2I.2f implementation/docs commit they should be synced to the final commit HEAD. `safe-cumulative` can be ignored.
3. **Hardware-test Phase 2I.2f next.** Local mobile typecheck and `git diff --check` passed; after commit/sync, hand the setup flow to Mohammad for hardware QA before starting the next memorization slice.
