...
**Last updated: 2026-04-29 (Phase 2I.2d Ready to Recite local validation passed; hardware QA pending; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  ...
  3. After Mohammad reports hardware QA, update `TODO.md` and this handoff to mark Phase 2I.2d hardware QA passed.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, and 2I.2c Recitation Check are implemented, validated, and hardware-tested. 2I.2d Ready to Recite is implemented and locally validated with hardware QA pending. Continue later with View in Full Mushaf and the remaining session parity rows only after 2I.2d is tested or Mohammad explicitly reorders. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, and mark-complete persistence. | `memorization.tsx`, maybe `mushaf.tsx` route params | Existing progress/daily-progress routes | Medium session-state risk | 2I.2d mobile typecheck and `git diff --check` passed; 2I.2a/2I.2b/2I.2c mobile typecheck / `git diff --check` passed and Mohammad hardware QA passed |
...
| **2I.2d Ready to Recite** | 🟡 local validation passed; hardware QA pending | this local-validation commit | Session-complete `Ready to Recite?` sheet; `Recite to Teacher` opens existing Recitation Check, `Recite to NoorPath` starts existing recite mode from the assigned first ayah and returns to score-card check |
...
## 7. Next: Phase 2I.2d Hardware QA

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, and Phase 2I.2c are complete and hardware-tested. Phase 2I.2d Ready to Recite is implemented and locally validated with hardware QA pending.
...
- **Phase 2I next** — Hardware-test the Ready to Recite modal, then continue memorization parity completion with View in Full Mushaf and a visible memorization settings/defaults page only after Mohammad confirms 2I.2d.
...
2. **Check git state.** `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `94f56029ca5fc5697b07ffbe5d7f2d90152331bb` at Phase 2I.2d start. After this Phase 2I.2d implementation/docs commit, all four refs should be synced to the final local-validation HEAD. `safe-cumulative` can be ignored.
3. **Hardware-test Phase 2I.2d after commit/sync.** Finish a session, confirm `Ready to Recite?`, test `Recite to Teacher`, test `Recite to NoorPath` through score-card Recitation Check, and smoke Save & Leave / Leave without saving / Keep Practicing.
4. **After hardware QA passes, update docs.** Mark 2I.2d hardware QA passed in `TODO.md` and this handoff before starting the next slice.
