...
**Last updated: 2026-04-29 (Phase 2I.2f pre-session setup hardware QA passed; next is Phase 2I.2g bookmark/resume per session; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  1. Start Phase 2I.2g Bookmark / resume per session in the active mobile app.
  2. Persist a per-child memorization session bookmark in AsyncStorage as session state changes: surah, fromAyah, toAyah, currentAyah, repeatCount, autoAdvance, cumulativeReview, reviewRepeatCount, review-only flag if needed, and savedAt.
  3. Add a "Resume where you left off" card to Memorize discovery that hydrates the bookmark, restores setup/session settings, and starts at the saved ayah without breaking today's/current/next freeze semantics.
  4. Clear or replace the bookmark when the saved session is completed, when `Leave without saving` intentionally discards it, or when a newer session supersedes it.
  5. Preserve all 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e/2I.2f behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, mark-complete persistence, and daily-card freeze semantics.
  6. Do not start 2I.2h, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2g is implemented, documented, validated, and hardware-tested unless Mohammad explicitly reorders.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, 2I.2f Pre-session setup, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are implemented, validated, and hardware-tested. Next slice is 2I.2g Bookmark / resume per session. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, Just Get Tested, setup behavior, daily-card freeze semantics, and mark-complete persistence. | `memorization.tsx`, `src/lib/settings.ts`, maybe `mushaf.tsx` route params later | Existing progress/daily-progress routes; AsyncStorage bookmark | Medium session-state risk | 2I.2a-2I.2f mobile typecheck / `git diff --check` passed and Mohammad hardware QA passed; skip-gap/freeze focused API typecheck, mobile typecheck, `git diff --check`, and Mohammad hardware QA passed |
...
## 7. Next: Phase 2I.2g Bookmark / Resume Per Session

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, Phase 2I.2e, Phase 2I.2f, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are complete and hardware-tested. The current active implementation item is Phase 2I.2g Bookmark / resume per session.
...
- **Current next** — Implement Phase 2I.2g Bookmark / resume per session: persist per-child session bookmark state, add a discovery resume card, hydrate saved settings/range/current ayah, and clear stale bookmarks on complete/discard/supersede.
...
2. **Check git state.** At Phase 2I.2f hardware-QA docs start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `dc7bf1436b86924762be271467d74f854cc08db1`; after this docs update they should be synced to the final docs HEAD. `safe-cumulative` can be ignored.
3. **Start Phase 2I.2g next.** Implement the per-session bookmark/resume flow before starting View in Full Mushaf or any later 2I/2J/2K/TestFlight work.
