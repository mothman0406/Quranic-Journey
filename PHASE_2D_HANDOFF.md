...
**Last updated: 2026-04-29 (Today's/Current work freeze correction hardware QA passed; next is Phase 2I.2f pre-session setup; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  1. Start Phase 2I.2f Pre-session setup in the active mobile app.
  2. Add a setup phase before memorization playback with bounded From/To ayah controls, repeat count 1-10, auto-advance toggle, cumulative review toggle, review repeat count stepper when cumulative review is on, and surah navigation/dropdown.
  3. Include a clear `Start - Ayah X to Y` CTA and keep a `Just Get Tested` shortcut for review-only assignments.
  4. Preserve 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e behavior, today's/current/next card semantics, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
  5. Do not start 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2f is implemented, documented, validated, and hardware-tested unless Mohammad explicitly reorders.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are implemented, validated, and hardware-tested. Next slice is 2I.2f Pre-session setup before playback. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, Just Get Tested, daily-card freeze semantics, and mark-complete persistence. | `memorization.tsx`, maybe `mushaf.tsx` route params later | Existing progress/daily-progress routes | Medium session-state risk | 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e mobile typecheck / `git diff --check` passed and Mohammad hardware QA passed; skip-gap/freeze focused API typecheck, mobile typecheck, `git diff --check`, and Mohammad hardware QA passed |
...
## 7. Next: Phase 2I.2f Pre-session Setup

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, Phase 2I.2e, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are complete and hardware-tested. The current active implementation item is Phase 2I.2f Pre-session setup.
...
- **Current next** — Implement Phase 2I.2f Pre-session setup in the active mobile app: bounded From/To ayah controls, repeat count, auto-advance, cumulative review, review repeat count, and surah navigation/dropdown before playback starts.
...
2. **Check git state.** After freeze-correction implementation commit `c479103`, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `c479103136da96ec4b8da9296c2b2ec23e1600b2`; after this docs update they should be synced to the final docs HEAD. `safe-cumulative` can be ignored.
3. **Start Phase 2I.2f next.** Implement the pre-session setup phase before memorization playback, preserving all hardware-tested 2I.2 save/leave/check/ready/tested and daily-card freeze behavior.
