...
**Last updated: 2026-04-29 (Phase 2I.2e Just Get Tested hardware QA passed; Next up skip-gap fix locally validated; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Next up skip-gap implementation notes: backend dashboard scheduling now bases `Next up` and full-surah completion on the explicit contiguous memorized ayah prefix instead of raw `versesMemorized` counts, so a saved non-contiguous range cannot make ayahs 17-49 look complete after only ayahs 1-16 were learned. Dashboard daily-progress selection now prefers the earliest matching workflow row over newer duplicate same-day rows, `POST /daily-progress` reuses an existing same-day row instead of creating another memorization row for the same date, and `POST /memorization` daily-progress auto-updates select the row overlapping the just-saved session instead of blindly updating the newest row. Local validation passed with focused API typecheck, mobile typecheck, and `git diff --check`.
- Exact next checklist:
  1. Validate the skip-gap fix with focused API typecheck if available, `cd artifacts/noor-mobile && npx tsc --noEmit`, and `git diff --check`; do not run the full root typecheck.
  2. Commit on `main`, push `origin main`, fast-forward `feature/main-working-branch` onto `main`, push `origin feature/main-working-branch`, and switch back to `main`.
  3. Hardware-test the reported gap scenario: after ayahs 1-16 are saved, confirm `Next up` shows the first missing contiguous range (17 onward, not 50-76); if an older duplicate already caused a gap, confirm the app routes back to fill that missing gap before later ayahs.
  4. Smoke 2I.2e Just Get Tested, 2I.2a completed-to saves, 2I.2b leave options, 2I.2c Recitation Check, and 2I.2d Ready to Recite after the backend fix.
  5. Do not start 2I.2f, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until this skip-gap fix is validated and shipped or Mohammad explicitly reorders.
...
- Memorization session/settings parity: mobile now has Pause & Save completed-to picking, Save & Leave / Leave without saving / Keep Practicing, Recitation Check with score card, Ready to Recite, and Just Get Tested implemented and hardware-tested. The Next up skip-gap backend fix is locally validated and awaiting hardware QA. It still needs View in Full Mushaf, bookmark/resume clarity, setup controls for ayah range/repeats/auto-advance/cumulative review/review repeat count, skip controls, richer ayah/word actions, recite-mode page helpers, celebration overlays, and a visible memorization settings page for defaults.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up, 2I.2a Pause & Save, 2I.2b leave semantics, 2I.2c Recitation Check, 2I.2d Ready to Recite, and 2I.2e Just Get Tested are hardware-tested; the Next up skip-gap backend fix is locally validated and awaiting hardware QA; View in Full Mushaf, setup/session settings, and a visible memorization settings/defaults page still need implementation.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, 2I.2c Recitation Check, 2I.2d Ready to Recite, and 2I.2e Just Get Tested are implemented, validated, and hardware-tested. The Next up skip-gap backend fix is locally validated and awaiting hardware QA. Continue later with View in Full Mushaf and the remaining session parity rows only after this fix ships or Mohammad explicitly reorders. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, and mark-complete persistence. | `memorization.tsx`, maybe `mushaf.tsx` route params; skip-gap fix touches API dashboard/daily-progress/memorization routes | Existing progress/daily-progress routes | Medium session-state risk | 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e mobile typecheck / `git diff --check` passed and Mohammad hardware QA passed; skip-gap focused API typecheck, mobile typecheck, and `git diff --check` passed |
...
| **Next up skip-gap backend fix** | 🟡 locally validated; hardware QA pending | local changes | Keeps `Next up` contiguous from explicit memorized ayahs, avoids newest duplicate daily-progress rows, and reuses same-day daily-progress rows instead of creating duplicate memorization targets |
...
## 7. Next: Commit And Ship Next Up Skip-Gap Fix

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, and Phase 2I.2e are complete and hardware-tested. The current active follow-up is the backend Next up skip-gap fix.
...
- **Current next** — Commit, sync, and hardware-test the locally validated Next up skip-gap backend fix. Confirm ayahs 1-16 lead to the first missing contiguous range instead of jumping to 50-76, and confirm existing gaps are routed back before later ranges.
- **Phase 2I next** — Continue memorization parity completion with View in Full Mushaf and a visible memorization settings/defaults page only after the skip-gap fix is shipped or Mohammad explicitly reorders.
...
2. **Check git state.** After the Phase 2I.2e implementation commit, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `e654ceb4e66cc0f3b712d38a1d1480dddce8dda7`. The Next up skip-gap fix is local, validated, and uncommitted pending sync. `safe-cumulative` can be ignored.
3. **Commit and sync the skip-gap fix.** Local validation passed with focused API typecheck, mobile typecheck, and `git diff --check`; do not run full root typecheck. Commit on `main`, push `origin main`, fast-forward `feature/main-working-branch`, push it, and switch back to `main`.
4. **Hardware-test the skip-gap fix next.** After ayahs 1-16 are saved, confirm `Next up` shows the first missing contiguous range instead of jumping to 50-76; if a gap already exists, confirm the app routes back to fill that gap before later ranges. Also smoke Just Get Tested and 2I.2a/2I.2b/2I.2c/2I.2d behavior.
