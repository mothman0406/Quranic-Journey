...
**Last updated: 2026-04-30 (Phase 2I.2i Skip repeat / Skip ayah committed/synced after local validation; hardware QA pending; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  1. Hardware-test Skip Repeat in single-phase repeat sessions and cumulative review, plus Skip Ayah in single-phase, at the final ayah, and during cumulative review.
  2. Preserve review-only assignments, `Just Get Tested`, bookmark/resume, Pause & Save, Save & Leave, Ready to Recite, Recitation Check, View in Full Mushaf, daily-card freeze semantics, ayah/page modes, cumulative review, audio highlighting, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence.
  3. Do not start 2I.2j, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2i is implemented, documented, validated, and hardware-tested unless Mohammad explicitly reorders.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up, 2I.2a Pause & Save, 2I.2b leave semantics, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, 2I.2f Pre-session setup, 2I.2g bookmark/resume, 2I.2h View in Full Mushaf, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are hardware-tested; Skip repeat / Skip ayah is implemented and locally validated with hardware QA pending; remaining session settings/defaults, richer ayah/word actions, recite-mode helpers, celebration states, and a visible memorization settings/defaults page still need implementation.
...
| 2I.2 | In progress Apr 30: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, 2I.2f Pre-session setup, 2I.2g Bookmark / resume per session, 2I.2h View in Full Mushaf, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are implemented, validated, and hardware-tested. 2I.2i Skip repeat / Skip ayah is implemented and locally validated, with hardware QA pending. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, Just Get Tested, setup behavior, bookmark semantics, daily-card freeze semantics, and mark-complete persistence. | `memorization.tsx`, `src/lib/settings.ts`, `mushaf.tsx` route params | Existing progress/daily-progress routes; AsyncStorage bookmark; existing Mushaf route | Medium session/navigation risk | 2I.2a-2I.2h mobile typecheck / `git diff --check` passed and Mohammad hardware QA passed; 2I.2i mobile typecheck / `git diff --check` passed, hardware QA pending |
...
| **2I.2i Skip repeat / Skip ayah** | 🟡 local validation passed; hardware QA pending | pending | Session controls add Skip Repeat / Skip Ayah; single-phase skips repeat passes or advances ayah/completion; cumulative skips pass or exits/advances review without corrupting the single-phase cursor |
...
## 7. Next: Hardware-test Phase 2I.2i Skip Repeat / Skip Ayah

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, Phase 2I.2e, Phase 2I.2f, Phase 2I.2g, Phase 2I.2h, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are complete and hardware-tested. Phase 2I.2i Skip repeat / Skip ayah is implemented, locally validated, committed, and synced; it needs hardware QA before the next slice starts.
...
- **Current next** — Hardware-test Skip Repeat / Skip Ayah before starting 2I.2j.
- **Phase 2I next** — After 2I.2i is hardware-tested, continue memorization parity completion with ayah/word actions, recite-mode helpers, confetti, discovery polish, and a visible memorization settings/defaults page unless Mohammad explicitly reorders.
...
2. **Check git state.** At Phase 2I.2i implementation start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `2b8524f3f0cbbe8c5019e27d47682b6338f5edd3`. Implementation/docs commit `68b6769` was pushed to both branches before this docs-only sync; after this docs commit, all four refs should be synced to the final docs HEAD. `safe-cumulative` can be ignored.
3. **Hardware-test 2I.2i next.** After this local-validation commit is synced, hardware-test Skip repeat / Skip ayah before starting ayah sheets or any later 2I/2J/2K/TestFlight work.
