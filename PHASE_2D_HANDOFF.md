...
**Last updated: 2026-04-29 (Phase 2I.2g bookmark/resume implemented and locally validated; hardware QA still required before 2I.2h; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Phase 2I.2g implementation notes: mobile now persists a per-child memorization session bookmark to AsyncStorage as the active session range/current ayah/settings change. The bookmark stores surah, From/To, current ayah, repeat count, auto-advance, cumulative review, review repeat count, review-only flag, page range where available, and `savedAt`. Memorize discovery shows a distinct `Resume where you left off` card, and tapping it bypasses setup to restore the bookmark and start playback at the saved ayah with saved range/settings. Successful Recitation Check saves clear the bookmark, `Leave without saving` intentionally clears it, and starting a newer session replaces it. Local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`.
- Exact next checklist:
  1. Commit the validated 2I.2g code/docs on `main`, push `origin main`, fast-forward `feature/main-working-branch` onto `main`, push `origin feature/main-working-branch`, and switch back to `main`.
  2. Mohammad hardware-tests 2I.2g on iPhone: quit mid-session, return to Memorize, confirm `Resume where you left off`, tap it, land at the saved ayah with saved range/settings, then confirm successful save clears the card and `Leave without saving` discards it.
  3. Preserve all 2I.2a/2I.2b/2I.2c/2I.2d/2I.2e/2I.2f behavior, ayah/page modes, audio highlighting, cumulative review, reciter settings, playback speed, blind/blur modes, translation popup, recite mode, mark-complete persistence, and daily-card freeze semantics.
  4. Do not start 2I.2h, 2I.3/2I.4, 2J, 2K, or TestFlight readiness until 2I.2g is committed, synced, and hardware-tested unless Mohammad explicitly reorders.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker, 2I.2b Save & Leave / Leave without saving / Keep Practicing, 2I.2c Recitation Check, 2I.2d Ready to Recite, 2I.2e Just Get Tested, 2I.2f Pre-session setup, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are implemented, validated, and hardware-tested. 2I.2g Bookmark / resume per session is implemented and locally validated: per-child AsyncStorage bookmark, discovery `Resume where you left off` card, saved ayah/settings hydration, clear on save/discard, and replacement by newer sessions. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, Recitation Check, Just Get Tested, setup behavior, daily-card freeze semantics, and mark-complete persistence. | `memorization.tsx`, `src/lib/settings.ts`, maybe `mushaf.tsx` route params later | Existing progress/daily-progress routes; AsyncStorage bookmark | Medium session-state risk | 2I.2a-2I.2g mobile typecheck / `git diff --check` passed; 2I.2a-2I.2f and skip-gap/freeze Mohammad hardware QA passed; 2I.2g hardware QA pending |
...
| **2I.2g Bookmark / resume** | 🟡 local validation passed; hardware pending | pending commit | Per-child AsyncStorage bookmark saves active session range/current ayah/settings; discovery `Resume where you left off` card hydrates saved range/settings and starts at saved ayah; save/discard clears; newer sessions replace |
...
## 7. Next: Validate + Hardware-Test Phase 2I.2g Bookmark / Resume

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, Phase 2I.2a, Phase 2I.2b, Phase 2I.2c, Phase 2I.2d, Phase 2I.2e, Phase 2I.2f, the `Next up` skip-gap fix, and the Today's/Current work freeze correction are complete and hardware-tested. Phase 2I.2g Bookmark / resume per session is implemented and locally validated; branch sync and Mohammad's iPhone hardware QA are still required before 2I.2h or later work starts.
...
- **Current next** — Commit/sync both tracked branches, then hardware-test quit/resume/save-clear/discard-clear/new-session-replace on iPhone.
- **Phase 2I next** — After 2I.2g is hardware-tested, continue memorization parity completion with View in Full Mushaf, skip controls, ayah/word actions, recite-mode helpers, confetti, discovery polish, and a visible memorization settings/defaults page unless Mohammad explicitly reorders.
...
2. **Check git state.** At Phase 2I.2g implementation start, `main`, `origin/main`, `feature/main-working-branch`, and `origin/feature/main-working-branch` were synced to `22c766a9e3e2ba86ca9546954304812ef02d36c3`; after the 2I.2g implementation/docs commit they should be synced to the final local-validation HEAD. `safe-cumulative` can be ignored.
3. **Sync or hardware-test Phase 2I.2g next.** The per-session bookmark/resume flow is implemented and locally validated; sync branches if needed, then hardware-test it before starting View in Full Mushaf or any later 2I/2J/2K/TestFlight work.
