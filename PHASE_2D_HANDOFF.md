...
**Last updated: 2026-04-29 (Phase 2I.1 overview cards/save semantics are locally validated; Bug A is hardware-tested/shipped; Bug B diagnostic logs are in place pending Mohammad's repro capture; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Bug A hardware QA result: Mohammad hardware-tested the Memorize search keyboard fix after commit `6a9e70f` and reported it is shipped. Active branch/SHA at Bug B diagnostic-logging start: `main` at `6a9e70f`, with all four refs synced. The active mobile memorization screen now has temporary `[noor-mem]` diagnostic logs only around `beginSession`, Step 1 dashboard fallback, Step 2 verses/timings fetch, the verse-change cleanup/autoplay effect, and `playVerse`. These logs are intentionally temporary and must be removed in the same commit as the eventual Bug B root-cause fix. Local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Remote sync target after this diagnostics/docs commit: all four refs synced to the final diagnostic HEAD. Mohammad should reproduce the `Next up` tap and paste the `[noor-mem]` lines before any logic patch is attempted.
- Hardware bugs found Apr 29 (block Phase 2I.1 QA): Bug A is the Memorize surah search keyboard sticking open; it is hardware-tested/shipped. Bug B is the Memorize `Next up` card loading death spiral; diagnostic logs are in place and the next step is Mohammad's repro/capture. Do not patch blindly. Remove any diagnostic logs in the same commit as the root-cause fix. Do not touch tajweed or tighten the recite matcher.
- QA status: Phase 2D memorization core through Slice 5b, Phase 2E dashboard polish, Phase 2F target-setting UI, Phase 2G.1 diagnostic cleanup, Phase 2G.2 mobile IA shell, Phase 2G.3 shared screen primitives, Phase 2H.1 mobile onboarding/profile management, Phase 2H.2 mobile dashboard parity content, Phase 2H.3 settings/targets convergence, Phase 2H.4 review essentials parity, Phase 2H.5 reading essentials parity, and Phase 2H.6 memorization discovery parity are hardware-tested. Phase 2G.4 local validation passed for API/codegen/mobile with `/Users/mothmanaurascape.ai/Library/pnpm/pnpm --filter @workspace/api-spec run codegen`, `/Users/mothmanaurascape.ai/Library/pnpm/pnpm run typecheck:libs`, `/Users/mothmanaurascape.ai/Library/pnpm/pnpm --filter @workspace/api-server run typecheck`, and `cd artifacts/noor-mobile && npx tsc --noEmit`. Phase 2H.1 local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; follow-up onboarding polish also passed the same two checks. Phase 2H.2 local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Phase 2H.3 local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Phase 2H.4 local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Phase 2H.5 local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Phase 2H.6 local validation passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`. Phase 2I.1 overview-card validation and save-semantics follow-up validation both passed with `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check`; hardware QA is pending. Full root `/Users/mothmanaurascape.ai/Library/pnpm/pnpm run typecheck` was previously attempted, but it stops in unrelated frozen/reference UI areas (`artifacts/noor-path/src/components/ui/button-group.tsx`, `artifacts/noor-path/src/components/ui/calendar.tsx`, `artifacts/mockup-sandbox/src/components/ui/calendar.tsx`, `artifacts/mockup-sandbox/src/components/ui/spinner.tsx`) on React type/ref baseline errors.
...
- Exact next checklist:
  1. Mohammad hardware-tests Phase 2I.1: open Memorize from dashboard; confirm `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; confirm cumulative recitation days follow the corrected 4-new / block-cumulative / 4-new / block-cumulative / full-cumulative / final-test cadence when a multi-day surah reaches those checkpoints; complete today's/current work; choose Needs Work/Good/Excellent and save; confirm `Today's work` stays the whole assignment, `Current work` advances through unfinished items inside today's assignment, and `Current work` only shows `Complete` after the last item in today's work is done; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today remains today; return to dashboard and confirm dashboard refresh; smoke search/filter/resume/session flow.
  2. After hardware QA passes, document the result, then continue Phase 2I.2 Pause/save and session action parity.
  3. Keep Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf as the next broader 2I.2 work; do not fold them into the already-implemented save/rating follow-up.
  4. Do not start TestFlight readiness until Phase 2I, 2J, and 2K are complete and hardware-tested. Do not tighten recite matcher behavior and do not touch tajweed implementation unless only documenting it as missing/backlogged.
...
- Memorization session/settings parity: mobile still needs the web's Pause & Save, Save & Leave, Leave without saving, View in Full Mushaf, bookmark/resume clarity, teacher/app recitation handoff, clearer finish-recitation affordances for review-only intra-surah days, setup controls for ayah range/repeats/auto-advance/cumulative review/review repeat count, and a visible memorization settings page for defaults.
...
| **2I.1 Memorization overview cards + save semantics** | ✅ local validation before scheduler fix; hardware QA pending | current Phase 2I.1 cumulative-scheduler fix HEAD | Web-style `Today's work`, `Current work`/`Recitation Focus`, and `Next up` cards backed by dashboard work/progress/next fields, plus rating-before-save, unioned ayah persistence, full-surah-based status, daily-progress sync, and corrected intra-surah cumulative recitation cadence |
...
## 7. Next: Phase 2I.1 Hardware QA

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, and Phase 2H.1-2H.6 are complete and hardware-tested. Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up are locally implemented, and the cumulative recitation scheduler has been corrected during QA. Next recommended work is Phase 2I.1 hardware QA, then Phase 2I.2 Pause & Save/session action parity.
...
- **Phase 2I next** — Memorization parity completion: hardware-test the locally implemented three cards plus save/rating semantics, then build pause/save and leave flows, View in Full Mushaf, and a visible memorization settings/defaults page.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current Bug B diagnostic-logging commit. Start new work from `main`; `safe-cumulative` can be ignored.
3. **Collect Bug B logs.** Mohammad should open Memorize, tap `Next up`, capture iPhone/Metro console output, and paste back only the `[noor-mem]` lines plus what the screen showed.
4. **Analyze and fix Bug B.** Use the logs to identify which lifecycle step hangs, then ship a root-cause fix in a separate commit that removes all temporary `[noor-mem]` logs in the same patch.
5. **After Bug B is fixed and hardware-confirmed, hardware-test Phase 2I.1.** Open Memorize from dashboard; confirm the three cards; confirm multi-day cumulative recitation follows the corrected four-new/block-cumulative/two-blocks-full-cumulative/separate-final-test cadence; complete/rate/save today's work; confirm today's card remains the same assignment marked done/in-progress; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today's card remains today's card; return to dashboard and confirm refresh; smoke search/filter/resume/session flow.
6. **After hardware QA passes, document the result and continue Phase 2I.2.** Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf are still pending.
7. **Preserve sensitive behavior.** Do not touch tajweed except to document it as backlogged, and do not tighten recite matcher behavior.
8. **Run `cd artifacts/noor-mobile && npx tsc --noEmit` and `git diff --check` after future mobile/docs changes.**
9. **Keep future slices JS-only unless explicitly approved.** Do not touch tajweed. Do not tighten recite matching. Do not add native dependencies unless Mohammad explicitly approves a rebuild.
10. **Update `TODO.md` and this handoff after meaningful work** with current date, branch/SHA, remote sync status, QA status, and the exact next checklist.
