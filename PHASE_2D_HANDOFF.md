...
**Last updated: 2026-04-29 (Phase 2I.1 overview cards/save semantics are locally validated; Bug A/B hardware-tested/shipped; Needs Work → Practice same-target loader fix locally validated, hardware QA pending; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Hardware bugs found Apr 29: Bug A was the Memorize surah search keyboard sticking open; Bug B was the Memorize `Next up` card loading death spiral; Bug C was the Needs Work → Practice same-target loader. Bug A/B are hardware-tested/shipped by Mohammad, and Bug C is locally validated pending hardware QA. Do not touch tajweed or tighten the recite matcher.
...
- Exact next checklist:
  ...
  3. After hardware QA passes, document the result, then continue Phase 2I.2 Pause/save and session action parity.
  4. Keep Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf as the next broader 2I.2 work; do not fold them into the already-implemented save/rating follow-up.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current Bug C same-target practice loader commit. Start new work from `main`; `safe-cumulative` can be ignored.
3. **Hardware-test the Bug C fix first.** Save a memorization range as Needs Work, return to Memorize, tap `Practice` for extra practice on that same surah/range, and confirm the session loads instead of spinning forever.
4. **Hardware-test Phase 2I.1.** Open Memorize from dashboard; confirm the three cards; complete today's/current work; choose Needs Work / Good / Excellent and save; confirm `Today's work` stays the same assignment and shows done/in-progress; confirm `Current work`/`Recitation Focus` advances through unfinished items inside today's assignment and only shows Complete when the last item in today's work is done; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today/current remain frozen correctly; return to dashboard and confirm refresh; smoke search/filter/resume/session flow.
5. **After hardware QA passes, document the result and continue Phase 2I.2.** Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf are still pending.
