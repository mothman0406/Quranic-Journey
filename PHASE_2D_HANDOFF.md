...
**Last updated: 2026-04-29 (Phase 2I.1 overview cards, save semantics/rating follow-up, and corrected intra-surah cumulative recitation schedule are implemented and locally validated; hardware QA remains pending, with beta still blocked by 2I/2J/2K parity before TestFlight readiness)**
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current Phase 2I.1 cumulative-scheduler fix/docs commit. Start new work from `main`; `safe-cumulative` can be ignored.
