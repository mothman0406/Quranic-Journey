...
**Last updated: 2026-04-29 (Phase 2H.6 memorization discovery parity is hardware-tested, but beta is not done; Phase 2I/2J/2K web-parity blockers must ship before TestFlight readiness)**
...
| Slice | Goal | Likely files | API deps | Risk | QA |
|---|---|---|---|---|---|
| 2I.1 | Add the web's top three mobile Memorize cards: `Today's work`, `Current work`/`Recitation Focus`, and `Next up`. They should show assignment range/type/status enough for parents/kids to understand the day, and route into the existing session engine without breaking current dashboard links. | `memorization.tsx`, maybe `src/lib/memorization.ts` | Existing dashboard/memorization/surahs endpoints | JS-only | Open Memorize from dashboard, see all three cards, start today's work, start current work, open next-up preview/action if enabled, return and refresh dashboard |
...
## 7. Next: Phase 2I.1 Memorization overview cards

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, and Phase 2H.1-2H.6 are complete and hardware-tested. Mohammad confirmed after 2H.6 QA that beta is still blocked by explicit web parity gaps. Next recommended work is Phase 2I.1 Memorization overview cards.
...
- **Phase 2I next** — Memorization parity completion: three cards, pause/save and leave flows, View in Full Mushaf, and a visible memorization settings/defaults page.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current pre-TestFlight parity-plan docs-sync commit. Start new work from `main`; `safe-cumulative` can be ignored.
3. **Start Phase 2I.1.** Inspect the frozen web `memorization.tsx` and `quran-memorize.tsx`, then add the top three cards to mobile Memorize: `Today's work`, `Current work`/`Recitation Focus`, and `Next up`.
4. **Preserve current behavior.** Keep dashboard entry behavior intact and do not rewrite the mobile memorization session engine.
