...
**Last updated: 2026-04-29 (Phase 2I.1 overview cards plus save semantics/rating follow-up are implemented and locally validated; hardware QA is pending, and beta is still blocked by 2I/2J/2K parity before TestFlight readiness)**
...
| **2I.1 Memorization overview cards + save semantics** | ✅ local validation; hardware QA pending | current Phase 2I.1 save-semantics HEAD | Web-style `Today's work`, `Current work`/`Recitation Focus`, and `Next up` cards backed by dashboard work/progress/next fields, plus rating-before-save, unioned ayah persistence, full-surah-based status, and daily-progress sync |
...
Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, and Phase 2H.1-2H.6 are complete and hardware-tested. Phase 2I.1 memorization overview cards plus save-semantics/rating follow-up are locally implemented. Next recommended work is Phase 2I.1 hardware QA, then Phase 2I.2 Pause & Save/session action parity.
...
3. **Hardware-test Phase 2I.1.** Open Memorize from dashboard; confirm the three cards; complete/rate/save today's work; confirm today's card remains the same assignment marked done/in-progress; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today's card remains today's card; return to dashboard and confirm refresh; smoke search/filter/resume/session flow.
4. **After hardware QA passes, document the result and continue Phase 2I.2.** Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf are still pending.
