...
**Last updated: 2026-04-29 (Phase 2I.2a Pause & Save implemented locally; hardware QA pending; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Exact next checklist:
  1. Hardware-test Phase 2I.2a on iPhone: start a normal memorization session, tap `Pause & Save`, pick a smaller completed-to ayah, continue to rating, choose Needs Work / Good / Excellent, save, and confirm only ayahs through the selected completed-to value are rated/memorized.
  2. Confirm partial Pause & Save keeps today's assignment `in_progress`, preserves the same `Today's work`, advances `Current work` only as far as the selected completed-to ayah allows, and refreshes the dashboard after returning.
  3. Repeat with `Full range` selected and confirm full-range save behavior still matches the Phase 2I.1 save semantics.
  4. Test a review-only `Recitation Focus`/`Recitation` assignment: confirm the session button says `Finish Recitation`, the Pause & Save sheet does not show the partial picker, `Go to Recitation Check` opens rating, and saving applies to the full assigned range.
  5. Confirm `Keep Practicing` closes both Pause & Save/rating sheets without saving.
  6. Smoke search/filter/resume/session flow, including Needs Work -> Practice loading, to ensure 2I.2a did not regress the cleared bugs.
  7. After Phase 2I.2a hardware QA passes, document the result, then continue Phase 2I.2b Save & Leave / Leave without saving / Keep practicing modal.
  8. Keep Pause & Save, Save & Leave, Leave without saving, and View in Full Mushaf as the next broader 2I.2 work; do not fold them into the already-implemented save/rating follow-up.
  9. Do not start TestFlight readiness until Phase 2I, 2J, and 2K are complete and hardware-tested. Do not tighten recite matcher behavior and do not touch tajweed implementation unless only documenting it as missing/backlogged.
...
- Phase 2I.2a Pause & Save is implemented locally and pending hardware QA: normal sessions can choose the completed-to ayah before rating, and review-only recitation assignments use an all-or-nothing `Go to Recitation Check` path.
...
- Memorization session/settings parity: mobile now has the first Pause & Save completed-to picker locally implemented and pending hardware QA; it still needs Save & Leave, Leave without saving, View in Full Mushaf, bookmark/resume clarity, teacher/app recitation handoff, a fuller recitation check phase with score card, clearer finish-recitation affordances for review-only intra-surah days, setup controls for ayah range/repeats/auto-advance/cumulative review/review repeat count, and a visible memorization settings page for defaults.
...
- Phase 2I Memorization parity completion is required before TestFlight: 2I.1 overview cards plus save-semantics/rating follow-up are hardware-tested, and 2I.2a Pause & Save is locally implemented pending hardware QA; session leave flow, View in Full Mushaf, setup/session settings, and a visible memorization settings/defaults page still need implementation.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker is locally implemented and validated. Continue with Save & Leave, Leave without saving, clearer review-only Finish Recitation affordances, and View in Full Mushaf from the memorization session. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence. | `memorization.tsx`, maybe `mushaf.tsx` route params | Existing progress/daily-progress routes | Medium session-state risk | 2I.2a local mobile typecheck and `git diff --check` passed; 2I.2a hardware QA pending |
...
| **2I.2a Pause & Save completed-to picker** | ✅ local validation; hardware QA pending | current Phase 2I.2a implementation/docs HEAD | Pause & Save sheet, completed-to ayah stepper, review-only Finish Recitation path, and partial-save rating/daily-progress semantics |
...
## 7. Next: Phase 2I.2a Hardware QA

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, and Phase 2I.1 are complete and hardware-tested. Phase 2I.2a is locally implemented and validated. Next recommended work is Phase 2I.2a hardware QA, then Phase 2I.2b Save & Leave / Leave without saving / Keep practicing.
...
- **Phase 2I next** — Memorization parity completion: build pause/save and leave flows, View in Full Mushaf, and a visible memorization settings/defaults page.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current Bug C hardware-confirmed docs commit. Start new work from `main`; `safe-cumulative` can be ignored.
3. **Hardware-test Phase 2I.2a.** Normal session: Pause & Save, pick a smaller completed-to ayah, rate/save, and confirm only selected ayahs are rated/memorized and today's work remains in progress. Full range: confirm existing full-save semantics. Review-only: confirm `Finish Recitation` -> `Go to Recitation Check` rates the full assigned range.
4. **After 2I.2a hardware QA passes, continue Phase 2I.2b.** Save & Leave, Leave without saving, and View in Full Mushaf are still pending.
