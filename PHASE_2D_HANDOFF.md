...
**Last updated: 2026-04-29 (Phase 2I.1 overview cards/save semantics are locally validated; Memorize search keyboard-dismiss Bug A is implemented for isolated hardware test; Next up loading Bug B still pending; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Hardware bugs found Apr 29 (block Phase 2I.1 QA): Bug A is the Memorize surah search keyboard sticking open; it is implemented locally as described above and now needs isolated iPhone confirmation. Bug B is the Memorize `Next up` card loading death spiral; it is still pending and should be diagnosed next with temporary `[noor-mem]` logs around `beginSession`, the Step 1 dashboard fetch effect, the Step 2 verses+timings effect, and the autoPlay branch of the `[playingVerseNumber]` cleanup effect. Remove any diagnostic logs in the same commit as the root-cause fix. Do not touch tajweed or tighten the recite matcher.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current Memorize search keyboard Bug A fix/docs commit. Start new work from `main`; `safe-cumulative` can be ignored.
3. **Hardware-test Bug A in isolation.** Open Memorize, tap the surah search field, confirm the keyboard dismisses when dragging the list, confirm the Done/return key dismisses it, and confirm filter/row taps are reachable while the keyboard is up.
4. **After Bug A hardware test passes, diagnose/fix Bug B.** Reproduce the Memorize `Next up` loading death spiral, add temporary `[noor-mem]` logs around `beginSession`, the Step 1 dashboard fetch effect, the Step 2 verses+timings effect, and the autoPlay branch of the `[playingVerseNumber]` cleanup effect, capture iPhone console output, fix the root cause, and remove logs in the same commit.
5. **After both bugs are cleared, hardware-test Phase 2I.1.** Open Memorize from dashboard; confirm the three cards; confirm multi-day cumulative recitation follows the corrected four-new/block-cumulative/two-blocks-full-cumulative/separate-final-test cadence; complete/rate/save today's work; confirm today's card remains the same assignment marked done/in-progress; confirm saved strength shows red/yellow/green where applicable; complete extra work if possible and confirm `Next up` can change while today's card remains today's card; return to dashboard and confirm refresh; smoke search/filter/resume/session flow.
