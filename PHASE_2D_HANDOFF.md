...
**Last updated: 2026-04-29 (Phase 2I.2b Save & Leave implemented locally; hardware QA next; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Memorization session/settings parity: mobile now has the first Pause & Save completed-to picker implemented and hardware-tested, and the Save & Leave / Leave without saving / Keep Practicing flow implemented and locally validated. It still needs 2I.2b hardware QA, View in Full Mushaf, bookmark/resume clarity, teacher/app recitation handoff, a fuller recitation check phase with score card, clearer finish-recitation affordances for review-only intra-surah days, setup controls for ayah range/repeats/auto-advance/cumulative review/review repeat count, and a visible memorization settings page for defaults.
...
| 2I.2 | In progress Apr 29: 2I.2a Pause & Save completed-to picker is implemented, validated, and hardware-tested; 2I.2b Save & Leave / Leave without saving / Keep Practicing is implemented locally and validated. Continue with 2I.2b hardware QA, then clearer/full Recitation Check, View in Full Mushaf, and the remaining session parity rows. Preserve ayah/page modes, audio highlighting, cumulative review, reciter/speed settings, blind/blur modes, translation popup, recite mode, and mark-complete persistence. | `memorization.tsx`, maybe `mushaf.tsx` route params | Existing progress/daily-progress routes | Medium session-state risk | 2I.2a local mobile typecheck, `git diff --check`, and Mohammad hardware QA passed; 2I.2b mobile typecheck and `git diff --check` passed, hardware QA pending |
...
## 7. Next: Phase 2I.2b Hardware QA, Then 2I.2c

Phase 2D, 2E, 2F, 2G.1, 2G.2, 2G.3, Phase 2H.1-2H.6, Phase 2I.1, and Phase 2I.2a are complete and hardware-tested. Phase 2I.2b Save & Leave / Leave without saving / Keep Practicing is implemented locally and validated. Next recommended work is commit/sync, Mohammad hardware QA for 2I.2b, then Phase 2I.2c Recitation Check phase with score card.
...
- **Phase 2I next** — Memorization parity completion: hardware-test the leave flow, then build Recitation Check, View in Full Mushaf, and a visible memorization settings/defaults page.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the final Phase 2I.2b local-validation commit after this handoff. Start new work from `main`; `safe-cumulative` can be ignored.
3. **Hardware-test Phase 2I.2b.** Verify active-session Back, `Keep Practicing`, normal `Save & Leave`, review-only `Save & Leave`, `Leave without saving`, and iOS swipe/back gesture behavior.
4. **If 2I.2b passes, start Phase 2I.2c.** Add the Recitation Check phase with score card while preserving the existing Pause & Save/rating save semantics.
