...
| Slice | Goal | Likely files | API deps | Risk | QA |
|---|---|---|---|---|---|
| 2I.1 | Implemented locally Apr 29: added top three mobile cards for `Today's work`, `Current work`/`Recitation Focus`, and `Next up`; added rating-before-save with Needs Work/Good/Excellent; saves unioned ayah progress with full-surah-based status; posts daily progress for today's target so the same card remains done/in-progress instead of visually advancing to tomorrow. | `memorization.tsx`, `src/lib/memorization.ts` | Existing dashboard/memorization/surahs, `POST /daily-progress`, `POST /memorization`; no generated edits | JS-only | Local mobile typecheck + `git diff --check` passed; hardware QA pending |
