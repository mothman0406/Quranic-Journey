...
**Last updated: 2026-04-29 (Phase 2I.1 overview cards/save semantics are locally validated; Bug A and Bug B are hardware-tested/shipped; temporary Bug B diagnostics removed; beta remains blocked by 2I/2J/2K parity before TestFlight readiness)**
...
- Hardware bugs found Apr 29: Bug A was the Memorize surah search keyboard sticking open; Bug B was the Memorize `Next up` card loading death spiral. Both are hardware-tested/shipped by Mohammad, and the temporary Bug B diagnostics are removed. Resume Phase 2I.1 hardware QA next. Do not touch tajweed or tighten the recite matcher.
...
2. **Check git state.** `main` and `feature/main-working-branch` should both contain the current Bug B diagnostic-cleanup/docs commit. Start new work from `main`; `safe-cumulative` can be ignored.
